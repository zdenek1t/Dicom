...
** Last Update:		$Author: smm $, $Date: 2000/06/15 03:58:53 $
** Source File:		$RCSfile: ctnthread.c,v $
** Revision:		$Revision: 1.9 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1.9 $ $RCSfile: ctnthread.c,v $";
...
#include <pthread.h>
//#include <synch.h>
static pthread_mutex_t mutex[FAC_MAXIMUM];
#endif
#endif
...
THR_Init()
{
    ...
    if (initialized) return THR_NORMAL;
    ...
    for (i = 0; i < (int) DIM_OF(mutex); i++) {
    	cond = pthread_mutex_init(&mutex[i], NULL);
...
THR_Shutdown()
{
    ...
    for (i = 0; i < (int) DIM_OF(mutex); i++) {
    	cond = pthread_mutex_destroy(&mutex[i]);
    	if (cond != 0) {
...
THR_ObtainMutex(int fac)
{
    ...
    if (!initialized) {
    	fprintf(stderr, "Threads not initialized in call to THR_ObtainMutex: exiting\n");
    	exit(1);
    }
    cond = pthread_mutex_lock(&mutex[fac]);
    if (cond != 0) {
...
THR_ReleaseMutex(int fac)
{
    ...
    cond = pthread_mutex_unlock(&mutex[fac]);
    if (cond != 0) {
