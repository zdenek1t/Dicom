...
** Last Update:		$Author: smm $, $Date: 1996/08/23 19:44:09 $
...
LST_NODE
*LST_Index(LST_HEAD ** list, int index);
char
*LST_Message(CONDITION cond);

#define LST_Top(x) 			LST_Head((x))
#define LST_Front(x) 		LST_Head((x))

#define LST_NORMAL			FORM_COND(FAC_LST, SEV_SUCC,  1)/* Normal return from LST package */
#define LST_LISTNOTEMPTY	FORM_COND(FAC_LST, SEV_ERROR, 3)/* Attempt to destroy list with elements */
