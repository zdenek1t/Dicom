 ...
 */
{
    if ((*list)->count != 0) return LST_LISTNOTEMPTY;
...
{
    if ((where != LST_K_BEFORE) && (where != LST_K_AFTER)) goto badend;
    ...
    if ((*list)->head == NULL) {/* if the list was empty     */
    	...
    	goto nocurrent;
    }else if ((CURRENT == (*list)->head) &&	(where == LST_K_BEFORE)) {	/* if at the head  and inserting BEFORE   */
    ...
    }else if ((CURRENT == (*list)->tail) &&	(where == LST_K_AFTER)) {	/* if at the tail and inserting AFTER    */
...
**/
{
    ...
    if ((dir != LST_K_BEFORE) && (dir != LST_K_AFTER)) goto baddir;
    if (CURRENT == NULL) goto nocurrent;
    if ((*list)->head == NULL) goto listempty;
...
LST_Sort(LST_HEAD ** list, size_t nodeSize, int (*compare) ())
{
    ...
    while ((n1 = LST_Dequeue(list)) != NULL) {
    	n2 = LST_Head(&head);
    	if (n2 != NULL) (void) LST_Position(&head, n2);
    	inserted = FALSE;
    	...
    	if (n2 == NULL) (void) LST_Enqueue(&head, n1);
    }
    **list = *head;
    return LST_NORMAL;
}
