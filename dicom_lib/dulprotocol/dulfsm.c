...
** Last Update:		$Author: smm $, $Date: 2002/08/16 17:41:09 $
** Source File:		$RCSfile: dulfsm.c,v $
** Revision:		$Revision: 1.42 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1.42 $ $RCSfile: dulfsm.c,v $";
...
static CONDITION
AE_1_TransportConnect(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_2_SendAssociateRQPDU(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_3_AssociateConfirmationAccept(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_4_AssociateConfirmationReject(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_5_TransportConnectResponse(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_6_ExamineAssociateRequest(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_7_SendAssociateAC(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AE_8_SendAssociateRJ(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);

static CONDITION
DT_1_SendPData(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** associatin, int nextState, void *params);
static CONDITION
DT_2_IndicatePData(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);

static CONDITION
AA_1_SendAAbort(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** associatin, int nextState, void *params);
static CONDITION
AA_2_CloseTransport(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_2_CloseTimeout(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_3_IndicatePeerAborted(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_4_IndicateAPAbort(PRIVATE_NETWORKKEY ** network,	PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_5_StopARTIMtimer(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_6_IgnorePDU(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_7_State13SendAbort(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AA_8_UnrecognizedPDUSendAbort(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);

static CONDITION
AR_1_SendReleaseRQ(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_2_IndicateRelease(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_3_ConfirmRelease(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_4_SendReleaseRP(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_5_StopARTIMtimer(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_6_IndicatePData(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_7_SendPDATA(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_8_IndicateARelease(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_9_SendAReleaseRP(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);
static CONDITION
AR_10_ConfirmRelease(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params);

#if 0
static CONDITION
requestAssociationTCP(PRIVATE_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, PRIVATE_ASSOCIATIONKEY ** association);
#endif
static CONDITION
sendAssociationRQTCP(PRIVATE_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, PRIVATE_ASSOCIATIONKEY ** association);
static CONDITION
sendAssociationACTCP(PRIVATE_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, PRIVATE_ASSOCIATIONKEY ** association);
static CONDITION
sendAssociationRJTCP(PRIVATE_NETWORKKEY ** network,	DUL_ABORTITEMS * abortItems, PRIVATE_ASSOCIATIONKEY ** association);
static CONDITION
sendAbortTCP(DUL_ABORTITEMS * abortItems, PRIVATE_ASSOCIATIONKEY ** association);
static CONDITION
sendReleaseRQTCP(PRIVATE_ASSOCIATIONKEY ** association);
static CONDITION
sendReleaseRPTCP(PRIVATE_ASSOCIATIONKEY ** association);
static CONDITION
sendPDataTCP(PRIVATE_ASSOCIATIONKEY ** association, DUL_PDVLIST * pdvList);
static CONDITION
writeDataPDU(PRIVATE_ASSOCIATIONKEY ** association, DUL_DATAPDU * pdu);
static void
...
static CONDITION
readPDUHead(PRIVATE_ASSOCIATIONKEY ** association, unsigned char *buffer, unsigned long maxlength, DUL_BLOCKOPTIONS block, int timeout, unsigned char *PDUtype, unsigned char *PDUreserved,	unsigned long *PDULength);
static CONDITION
readPDU(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, int timeout, unsigned char *buffer, unsigned long maxLength,	unsigned char *pduType, unsigned char *pduReserved,	unsigned long *pduLength);
static CONDITION
readPDUBody(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, int timeout,	unsigned char *buffer, unsigned long maxLength,	unsigned char *pduType, unsigned char *pduReserved,	unsigned long *pduLength);
static CONDITION
readPDUHeadTCP(PRIVATE_ASSOCIATIONKEY ** association, unsigned char *buffer, unsigned long maxLength, DUL_BLOCKOPTIONS block, int timeout, unsigned char *PDUtype, unsigned char *PDUreserved, unsigned long *PDULength);
static CONDITION
readPDUBodyTCP(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, int timeout, unsigned char *buffer, unsigned long maxLength, unsigned char *pduType, unsigned char *pduReserved, unsigned long *pduLength);
static CONDITION
/*defragmentTCP(int socket, DUL_BLOCKOPTIONS block, time_t timerStart,*/
defragmentTCP(PRIVATE_ASSOCIATIONKEY** association, DUL_BLOCKOPTIONS block, time_t timerStart, int timeout, void *b, unsigned long l, unsigned long *rtnLen);
...
CONDITION
translatePresentationContextList(LST_HEAD ** internalList, LST_HEAD ** SCUSCPRoleList, LST_HEAD ** userContextList);
...
DUL_InitializeFSM()
{
    unsigned long	      index, idx2;
    FSM_ENTRY			  * stateEntries;
    ...
    for (index = 0; index < DUL_NUMBER_OF_EVENTS * DUL_NUMBER_OF_STATES; index++) {
    	if (stateEntries[index].action != NOACTION) {
    		for (idx2 = 0; idx2 < DIM_OF(FSM_FunctionTable) && stateEntries[index].actionFunction == NULL; idx2++)
    			if (stateEntries[index].action == FSM_FunctionTable[idx2].action) {
    				stateEntries[index].actionFunction = FSM_FunctionTable[idx2].actionFunction;
    				(void) sprintf(stateEntries[index].actionName, "%.*s", sizeof(stateEntries[index].actionName) - 1, FSM_FunctionTable[idx2].actionName);
    			}
    	}
    	for (idx2 = 0; idx2 < DIM_OF(Event_Table) && strlen(stateEntries[index].eventName) == 0; idx2++) {
    		if (stateEntries[index].event == Event_Table[idx2].event)
    			(void) sprintf(stateEntries[index].eventName, "%.*s", sizeof(stateEntries[index].eventName) - 1, Event_Table[idx2].eventName);
		}
    }
    return DUL_NORMAL;
}
...
*/
CONDITION
PRV_StateMachine(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int event, int state, void *params)
{
    FSM_ENTRY		* entry;
    ...
    if (event < 0 || event >= DUL_NUMBER_OF_EVENTS) {
    	...
    	return COND_PushCondition(DUL_FSMERROR, DUL_Message(DUL_FSMERROR), state, event);
    }
...
#ifdef DEBUG
    if (debug) {
    	(void) fprintf(DEBUG_DEVICE, "================ PRV_StateMachine ================\n");
    ...
    if (entry->actionFunction != NULL){
    	...
    	(void) fprintf(stderr, "PRV_StateMachine: No action defined, state: %d event %d\n", state, event);
    	return COND_PushCondition(DUL_FSMERROR, DUL_Message(DUL_FSMERROR), state, event);
    }
}
...
AE_1_TransportConnect(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
  ...
  return 0;
    ...
    if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0) {
    	...
    	cond = COND_PushCondition(DUL_UNSUPPORTEDNETWORK, DUL_Message(DUL_UNSUPPORTEDNETWORK), (*network)->networkType);
    }
    return cond;
#endif
}
...
AE_2_SendAssociateRQPDU(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0) {
    	...
    	return COND_PushCondition(DUL_UNSUPPORTEDNETWORK, DUL_Message(DUL_UNSUPPORTEDNETWORK), (*network)->networkType);
    }
}
...
AE_3_AssociateConfirmationAccept(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    DUL_ASSOCIATESERVICEPARAMETERS		* service;
    CONDITION							cond;
    unsigned char						buffer[16384], pduType, pduReserve;
    ...
    service = (DUL_ASSOCIATESERVICEPARAMETERS *) params;
    cond = readPDU(association, DUL_BLOCK, 0, buffer, sizeof(buffer), &pduType, &pduReserve, &pduLength);
    if (cond != DUL_NORMAL)	return cond;

    if (debug) dump_pdu("Associate Accept", buffer, pduLength + 6);

    if (pduType == DUL_TYPEASSOCIATEAC) {
    	cond = parseAssociate(buffer, pduLength, &assoc);
    	if (debug) (void) fflush(DEBUG_DEVICE);
		...
		if ((service->acceptedPresentationContext = LST_Create()) == NULL)
			return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "AE_3_AssociateConfirmationAccept");
		...
		while (prvCtx != NULL) {
			...
			if (userPresentationCtx == NULL)
				return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "AE_3_AssociateConfirmationAccept", sizeof(*userPresentationCtx));
			...
			requestedPresentationCtx = findPresentationCtx( &service->requestedPresentationContext, prvCtx->contextID);
			...
			scuscpRole = findSCUSCPRole( &assoc.userInfo.SCUSCPRoleList, userPresentationCtx->abstractSyntax);
			if (scuscpRole != NULL) {
			...
			if (subItem != NULL) (void) strcpy(userPresentationCtx->acceptedTransferSyntax, subItem->data);

			if (LST_Enqueue(&service->acceptedPresentationContext, userPresentationCtx) != LST_NORMAL)
				return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "AE_3_AssociateConfirmationAccept");
		...
		strcpy(service->calledImplementationClassUID, assoc.userInfo.implementationClassUID.data);
		strcpy(service->calledImplementationVersionName, assoc.userInfo.implementationVersionName.data);
		(*association)->associationState = DUL_ASSOC_ESTABLISHED;
		(*association)->protocolState = nextState;
		return DUL_NORMAL;
    }else{
    	return COND_PushCondition(DUL_UNEXPECTEDPDU, DUL_Message(DUL_UNEXPECTEDPDU));
    }
}
...
AE_4_AssociateConfirmationReject(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    cond = readPDUBody(association, DUL_BLOCK, 0, buffer, sizeof(buffer), &pduType, &pduReserve, &pduLength);
    if (cond != DUL_NORMAL)	return cond;
    ...
    if (pduType == DUL_TYPEASSOCIATERJ) {
    	...
    	cond = COND_PushCondition(DUL_ASSOCIATIONREJECTED, DUL_Message(DUL_ASSOCIATIONREJECTED));
    }else{
    	cond = COND_PushCondition(DUL_UNEXPECTEDPDU, DUL_Message(DUL_UNEXPECTEDPDU));
    }
    return cond;
}
...
AE_6_ExamineAssociateRequest(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    if (pduType == DUL_TYPEASSOCIATERQ) {
    	if (debug) dump_pdu("Associate Request", buffer, pduLength + 6);

    	cond = parseAssociate(buffer, pduLength, &assoc);
    	if (debug) (void) fflush(DEBUG_DEVICE);

    	if (cond != DUL_NORMAL) {
    		if (cond == DUL_UNSUPPORTEDPEERPROTOCOL) (*association)->protocolState = STATE3;	/* Make it look OK */
    		return cond;
    	}
    	...
    	if ((service->requestedPresentationContext = LST_Create()) == NULL)
    		return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "AE_6_ExamineAssociateRequest");
    	if (translatePresentationContextList(&assoc.presentationContextList, &assoc.userInfo.SCUSCPRoleList, &service->requestedPresentationContext) != DUL_NORMAL)
			return COND_PushCondition(DUL_PCTRANSLATIONFAILURE, DUL_Message(DUL_PCTRANSLATIONFAILURE), "AE_6_ExamineAssociateRequest");

    	service->peerMaxPDU = assoc.userInfo.maxLength.maxLength;
    	(*association)->maxPDV = assoc.userInfo.maxLength.maxLength;
    	(*association)->maxPDVRequestor = assoc.userInfo.maxLength.maxLength;
    	strcpy(service->callingImplementationClassUID, assoc.userInfo.implementationClassUID.data);
    	strcpy(service->callingImplementationVersionName, assoc.userInfo.implementationVersionName.data);
    	(*association)->associationState = DUL_ASSOC_ESTABLISHED;
...
static CONDITION
DT_1_SendPData(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    CONDITION		    cond;
    DUL_PDVLIST			* pdvList;
    ...
    if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0) {
    	...
    	return COND_PushCondition(DUL_UNSUPPORTEDNETWORK, DUL_Message(DUL_UNSUPPORTEDNETWORK), (*association)->networkType);
    }
}
/* DT_2_IndicatePData
**
...
DT_2_IndicatePData(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    cond = readPDUBody(association, DUL_BLOCK, 0, (*association)->fragmentBuffer, (*association)->fragmentBufferLength, &pduType, &pduReserved, &pduLength);
    if (cond != DUL_NORMAL)	return cond;
#ifdef DUMP_DATA_PDU
    if (debug) dump_data((*association)->fragmentBuffer, pduLength);
#endif
#ifdef BLOG
    if (blog) {
    	U32 type;
    	type = pduType;
    	BLG_Write(&(*association)->logHandle, type, (*association)->fragmentBuffer, pduLength);
    }
#endif
    ...
    if (length != 0) {
    	(void) fprintf(stderr, "PDV Lengths don't add up correctly: %ld pdvs\n", pdvCount);
    	(void) fprintf(stderr, "In DT_2_IndicatePData.  This probably indicates a");
    	(void) fprintf(stderr, " malformed P DATA PDU.\n");
    	return COND_PushCondition(DUL_ILLEGALPDU, DUL_Message(DUL_ILLEGALPDU),(unsigned long) pduType);
    }
...
/*
    ...
    if (length != 0) {
		(void)fprintf(stderr, "DT_2_IndicatePData:PDV Lengths don't add up correctly: %d pdvs\n", pdvCount);
		return COND_PushCondition(DUL_ILLEGALPDU, DUL_Message(DUL_ILLEGALPDU),(unsigned long)pduType);
    }
*/
...
AR_2_IndicateRelease(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    if (blog) {
    	...
    	BLG_Write(&(*association)->logHandle, type, &dummy, (unsigned long) sizeof(dummy));
    }
#endif
...
static CONDITION
AA_1_SendAAbort(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    DUL_ABORTITEMS		   	abortItems;
    CONDITION				cond;
...
AA_3_IndicatePeerAborted(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    closeTransport(association);
    (*association)->protocolState = nextState;
    return COND_PushCondition(DUL_PEERABORTEDASSOCIATION, DUL_Message(DUL_PEERABORTEDASSOCIATION));
...
AA_6_IgnorePDU(PRIVATE_NETWORKKEY ** network, PRIVATE_ASSOCIATIONKEY ** association, int nextState, void *params)
{
    ...
    unsigned long		 PDULength, l;
    CONDITION			 cond;

    (*association)->protocolState = nextState;
    cond = readPDUHead(association, buffer, sizeof(buffer), DUL_NOBLOCK, PRV_DEFAULTTIMEOUT, &PDUType, &PDUReserved, &PDULength);
    if (cond != DUL_NORMAL) return cond;

    while (PDULength > 0 && cond == DUL_NORMAL) {
    	if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0) {
    		cond = defragmentTCP(association, DUL_NOBLOCK, (*association)->timerStart, (*association)->timeout, buffer, sizeof(buffer), &l);
    		if (cond != DUL_NORMAL) return cond;
					  ...
					  PRIVATE_ASSOCIATIONKEY ** association)
{
    ...
    if (s < 0) return COND_PushCondition(DUL_TCPINITERROR, DUL_Message(DUL_TCPINITERROR), strerror(errno));
    ...
    hp = gethostbyname(node);
#endif
    if (hp == NULL) return COND_PushCondition(DUL_UNKNOWNHOST, DUL_Message(DUL_UNKNOWNHOST), node);
...
sendAssociationRQTCP(PRIVATE_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, PRIVATE_ASSOCIATIONKEY ** association)
{
    PRV_ASSOCIATEPDU			associateRequest;
    ...
    cond = constructAssociatePDU(params, DUL_TYPEASSOCIATERQ, &associateRequest);
    if (cond != DUL_NORMAL)	return cond;
    ...
    if (associateRequest.length + 2 <= sizeof(buffer)){
    	...
    	if (b == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "sendAssociationRQTCP", associateRequest.length + 2);
    }
    cond = streamAssociatePDU(&associateRequest, b, associateRequest.length + 2, &length);
...
#if 0
#ifdef _MSC_VER
    nbytes = send((*association)->networkSpecific.TCP.socket, b, associateRequest.length + 6, 0);
#else
    nbytes = write((*association)->networkSpecific.TCP.socket, b, associateRequest.length + 6);
#endif
#endif

    nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, b, associateRequest.length + 6);
    if ((unsigned long) nbytes != associateRequest.length + 6) {
    	return COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "requestAssociationTCP");
    }
    if (b != buffer) CTN_FREE(b);
    return DUL_NORMAL;
}
...
sendAssociationACTCP(PRIVATE_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, PRIVATE_ASSOCIATIONKEY ** association)
{
    PRV_ASSOCIATEPDU	    associateReply;
    unsigned char			buffer[4096], *b;
    unsigned long			length;
    int						nbytes;
    CONDITION				cond;
    ...
    localService = *params;
    cond = constructAssociatePDU(&localService, DUL_TYPEASSOCIATEAC, &associateReply);
    if (cond != DUL_NORMAL)	return cond;
    ...
    if (associateReply.length + 2 <= sizeof(buffer)){
		...
		if (b == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "ReplyAssociationTCP", associateReply.length + 2);
    }
    cond = streamAssociatePDU(&associateReply, b, associateReply.length + 2, &length);
...
#if 0
#ifdef _MSC_VER
    nbytes = send((*association)->networkSpecific.TCP.socket, b, associateReply.length + 6, 0);
#else
    nbytes = write((*association)->networkSpecific.TCP.socket, b, associateReply.length + 6);
#endif
#endif
    nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, b,	associateReply.length + 6);

    if ((unsigned long) nbytes != associateReply.length + 6) {
    	return COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "ReplyAssociationTCP");
    }
    if (b != buffer) CTN_FREE(b);
    return DUL_NORMAL;
}
...
sendAssociationRJTCP(PRIVATE_NETWORKKEY ** network, DUL_ABORTITEMS * abortItems, PRIVATE_ASSOCIATIONKEY ** association)
{
    ...
    if (pdu.length + 6 <= sizeof(buffer)){
    	...
    	if (b == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "ReplyAssociationTCP", pdu.length + 6);
    }
    cond = streamRejectReleaseAbortPDU(&pdu, b, pdu.length + 6, &length);
    if (cond == DUL_NORMAL) {
#if 0
#ifdef _MSC_VER
    	nbytes = send((*association)->networkSpecific.TCP.socket, b, pdu.length + 6, 0);
#else
    	nbytes = write((*association)->networkSpecific.TCP.socket, b, pdu.length + 6);
#endif
#endif

    	nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, b,	pdu.length + 6);
    	if ((unsigned long) nbytes != pdu.length + 6) {
    		cond = COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "sendAssociationRJTCP");
    	}
    }
    if (b != buffer) CTN_FREE(b);
    return cond;
...
sendAbortTCP(DUL_ABORTITEMS * abortItems, PRIVATE_ASSOCIATIONKEY ** association)
{
    ...
    if (pdu.length + 6 <= sizeof(buffer)){
    	...
    	b = CTN_MALLOC(pdu.length + 6);
    	if (b == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "sendAbortTCP", pdu.length + 6);
    }
    cond = streamRejectReleaseAbortPDU(&pdu, b, pdu.length + 6, &length);
    if (cond == DUL_NORMAL) {
#if 0
    	...
    	if ((unsigned long) nbytes != pdu.length + 6) {
    		cond = COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "sendAbortTCP");
    	}
    }
    if (b != buffer) CTN_FREE(b);
...
sendReleaseRQTCP(PRIVATE_ASSOCIATIONKEY ** association)
{
    ...
    if (pdu.length + 6 <= sizeof(buffer)){
    	...
    	b = CTN_MALLOC(pdu.length + 6);
    	if (b == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "sendReleaseRQTCP", pdu.length + 6);
    }
    cond = streamRejectReleaseAbortPDU(&pdu, b, pdu.length + 6, &length);
    if (cond == DUL_NORMAL) {
#if 0
    	...
    	nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, b,	pdu.length + 6);
    	if ((unsigned long) nbytes != pdu.length + 6) {
    		cond = COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "sendReleaseRQTCP");
    	}
    }
    if (b != buffer) CTN_FREE(b);
...
sendReleaseRPTCP(PRIVATE_ASSOCIATIONKEY ** association)
{
    ...
    if (pdu.length + 6 <= sizeof(buffer)){
		...
		b = CTN_MALLOC(pdu.length + 6);
		if (b == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "sendReleaseRPTCP", pdu.length + 6);
    }
    cond = streamRejectReleaseAbortPDU(&pdu, b, pdu.length + 6, &length);
    if (cond == DUL_NORMAL) {
#if 0
    	...
    	nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, b,	pdu.length + 6);
    	if ((unsigned long) nbytes != pdu.length + 6) {
    		cond = COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "sendReleaseRPTCP");
    	}
    }
    if (b != buffer) CTN_FREE(b);
...
{
    DUL_PDV			    * pdv;
    unsigned long		count, length, pdvLength;
    int			        localLast;
    CONDITION			cond;
    unsigned char	    *p;
    DUL_DATAPDU			dataPDU;
    CTNBOOLEAN			firstTrip;
    ...
    while (cond == DUL_NORMAL && count-- > 0) {
    	...
    	while ((firstTrip || (length > 0)) && (cond == DUL_NORMAL)) {
    		...
    		cond = constructDataPDU(p, pdvLength, pdv->pdvType, pdv->presentationContextID, localLast, &dataPDU);
...
writeDataPDU(PRIVATE_ASSOCIATIONKEY ** association,	 DUL_DATAPDU * pdu)
{
    ...
    if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0) {
    	...
    	if (debug) {
    		...
    		recordOutGoing(pdu->presentationDataValue.data, pdu->presentationDataValue.length - 2);
    	}
#endif
#if 0
#ifdef _MSC_VER
    	...
    	nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, head, length);

    	if ((unsigned long) nbytes != length)
    		return COND_PushCondition(DUL_TCPIOERROR, DUL_Message(DUL_TCPIOERROR), strerror(errno), "writeDataPDU");
#if 0
#ifdef _MSC_VER
    	...
    	nbytes = (*association)->proxy.writeFunction((*association)->proxy.proxyContext, pdu->presentationDataValue.data, pdu->presentationDataValue.length - 2);
...
closeTransportTCP(PRIVATE_ASSOCIATIONKEY ** association)
{
  (*association)->proxy.closeFunction((*association)->proxy.proxyContext);
...
CONDITION
PRV_NextPDUType(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, int timeout, unsigned char *pduType)
{
    CONDITION		cond;

    if ((*association)->inputPDU == NO_PDU) {
    	cond = readPDUHead(association, (*association)->pduHead, sizeof((*association)->pduHead), block, timeout, &(*association)->nextPDUType, &(*association)->nextPDUReserved, &(*association)->nextPDULength);
    	if (cond != DUL_NORMAL) return cond;
...
readPDU(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, int timeout, unsigned char *buffer, unsigned long maxLength,	unsigned char *pduType, unsigned char *pduReserved,	unsigned long *pduLength)
{
    ...
    cond = readPDUBody(association, block, timeout, buffer + sizeof((*association)->pduHead), maxLength - sizeof((*association)->pduHead), pduType, pduReserved, pduLength);
    return cond;
}
...
readPDUHead(PRIVATE_ASSOCIATIONKEY ** association, unsigned char *buffer, unsigned long maxLength, DUL_BLOCKOPTIONS block, int timeout, unsigned char *PDUType, unsigned char *PDUReserved,	unsigned long *PDULength)
{
    ...
    if ((*association)->inputPDU == NO_PDU) {
    	if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0)
    		cond = readPDUHeadTCP(association, buffer, maxLength, block, timeout, &(*association)->nextPDUType, &(*association)->nextPDUReserved, &(*association)->nextPDULength);
    	else
    		cond = COND_PushCondition(DUL_UNSUPPORTEDNETWORK, DUL_Message(DUL_UNSUPPORTEDNETWORK), (*association)->networkType);
    }
    ...
    if (cond == DUL_NORMAL) {
    	...
    	*PDULength = (*association)->nextPDULength;
    	if ((*PDUType == DUL_TYPEDATA) && (*PDULength > (*association)->maxPDVInput))
    		cond = COND_PushCondition(DUL_ILLEGALPDULENGTH, DUL_Message(DUL_ILLEGALPDULENGTH), *PDULength, (*association)->maxPDVInput, "readPDUHead");
    }
    return cond;
}
			...
			unsigned long *pduLength)
{
    ...
    if (strcmp((*association)->networkType, DUL_NETWORK_TCP) == 0){
    	cond = readPDUBodyTCP(association, block, timeout, buffer, maxLength, pduType, pduReserved, pduLength);
    }else{
		return COND_PushCondition(DUL_UNSUPPORTEDNETWORK, DUL_Message(DUL_UNSUPPORTEDNETWORK), (*association)->networkType);
    }
    (*association)->inputPDU = NO_PDU;
...
static CONDITION
readPDUHeadTCP(PRIVATE_ASSOCIATIONKEY ** association, unsigned char *buffer, unsigned long maxLength, DUL_BLOCKOPTIONS block, int timeout, unsigned char *type, unsigned char *reserved, unsigned long *pduLength)
{
    CONDITION		 		cond;
    unsigned long	        length;
    ...
    if (maxLength < 6)
    	return COND_PushCondition(DUL_CODINGERROR, DUL_Message(DUL_CODINGERROR), "buffer too small in readPDUTCPHead");

    if (timeout == PRV_DEFAULTTIMEOUT)
    	timeout = (*association)->timeout;

    cond = defragmentTCP(association, block, (*association)->timerStart, timeout, buffer, 6, &length);
    if (cond != DUL_NORMAL)	return cond;
    ...
    *reserved = *buffer++;
    ...
    if (!found)	return COND_PushCondition(DUL_UNRECOGNIZEDPDUTYPE, DUL_Message(DUL_UNRECOGNIZEDPDUTYPE), *type);
...
#ifdef DEBUG
    if (debug) (void) fprintf(DEBUG_DEVICE, "Read PDU HEAD TCP: type: %x, length: %ld (%lx)\n", *type, *pduLength, *pduLength);
#endif
    return DUL_NORMAL;
}
...
readPDUBodyTCP(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, int timeout, unsigned char *buffer, unsigned long maxLength, unsigned char *pduType, unsigned char *pduReserved, unsigned long *pduLength)
{
    ...
    *pduLength = (*association)->nextPDULength;
    ...
    if (timeout == PRV_DEFAULTTIMEOUT) timeout = (*association)->timeout;

    cond = defragmentTCP(association, block, (*association)->timerStart, timeout, buffer, (*association)->nextPDULength, &length);
    return cond;
}
...
static CONDITION
defragmentTCP(PRIVATE_ASSOCIATIONKEY ** association, DUL_BLOCKOPTIONS block, time_t timerStart, int timeout, void *p, unsigned long l, unsigned long *rtnLen)
{
    unsigned char			*b;
    int				        bytesRead;

    b = (unsigned char *) p;
    if (rtnLen != NULL)	*rtnLen = 0;
...
#ifdef SMM
    	if (timeToWait <= 0) return DUL_READTIMEOUT;
#endif
    	if ((*association)->proxy.pollFunction((*association)->proxy.proxyContext, timeToWait) == 0) return DUL_READTIMEOUT;
#if 0
    	if (!networkDataAvailable((*association)->networkSpecific.TCP.socket, timeToWait)) return DUL_READTIMEOUT;
#endif
    }
...
#endif
#endif
    	bytesRead = (*association)->proxy.readFunction((*association)->proxy.proxyContext, b, l);

    	if (bytesRead > 0) {
    		b += bytesRead;
    		l -= (unsigned long) bytesRead;
    		if (rtnLen != NULL) *rtnLen += (unsigned long) bytesRead;
    	} else {
...
dump_pdu(char *type, void *buffer, unsigned long length)
{
    ...
    while (length-- > 0) {
    	...
    	if ((++position) % 16 == 0) (void) fprintf(DEBUG_DEVICE, "\n");
    }
...
translatePresentationContextList(LST_HEAD ** internalList, LST_HEAD ** SCUSCPRoleList, LST_HEAD ** userContextList)
{
    ...
    DUL_PRESENTATIONCONTEXT			* userContext;
    DUL_SUBITEM						* subItem;
    ...
    while (context != NULL) {
    	...
    	if (userContext == NULL)
    		return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "translatePresentationContextList", sizeof(*userContext));
    	if ((userContext->proposedTransferSyntax = LST_Create()) == NULL)
    		return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "translatePresentationContextList");
    	...
    	if (subItem == NULL) return COND_PushCondition(DUL_PEERILLEGALXFERSYNTAXCOUNT, DUL_Message(DUL_PEERILLEGALXFERSYNTAXCOUNT), 0);
    	...
    	while (subItem != NULL) {
    		...
    		if (transfer == NULL)
				return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "translatePresentationContextList", sizeof(*transfer));

    		strcpy(transfer->transferSyntax, subItem->data);

    		if (LST_Enqueue(&userContext->proposedTransferSyntax, transfer) != LST_NORMAL)
    			return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "translatePresentationContextList");
...
findPresentationCtx( LST_HEAD ** list, DUL_PRESENTATIONCONTEXTID contextID)
{
    ...
    if ((ctx = LST_Head(list)) == NULL)	return NULL;

    (void) LST_Position(list, ctx);

    while (ctx != NULL) {
    	if (ctx->presentationContextID == contextID) return ctx;
    	ctx = LST_Next(list);
    }
    return NULL;
}
...
findSCUSCPRole(LST_HEAD ** list, char *abstractSyntax)
{
    ...
    role = LST_Head(list);
    if (role != NULL) (void) LST_Position(list, role);

    while (role != NULL) {
    	if (strcmp(role->SOPClassUID, abstractSyntax) == 0) return role;
    	role = LST_Next(list);
    }
    return NULL;
}
...
CONDITION
DULPRV_translateAssocReq(unsigned char *buffer, unsigned long pduLength, DUL_ASSOCIATESERVICEPARAMETERS * service)
{
    PRV_ASSOCIATEPDU				assoc;
    CONDITION 						cond;
    ...
    if ((service->requestedPresentationContext = LST_Create()) == NULL)
    	return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "DULPRV_translateAssocReq");
    if (translatePresentationContextList(&assoc.presentationContextList, &assoc.userInfo.SCUSCPRoleList, &service->requestedPresentationContext) != DUL_NORMAL)
    	return COND_PushCondition(DUL_PCTRANSLATIONFAILURE, DUL_Message(DUL_PCTRANSLATIONFAILURE), "DULPRV_translateAssocReq");
