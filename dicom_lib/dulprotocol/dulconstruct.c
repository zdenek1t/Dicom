...
static CONDITION
constructSubItem(char *name, unsigned char type, DUL_SUBITEM * applicationContext, unsigned long *rtnlen);
static CONDITION
...
static CONDITION
constructMaxLength(unsigned long maxPDU, DUL_MAXLENGTH * max, unsigned long *rtnLen);
static CONDITION
constructSCUSCPRoles(unsigned char type, DUL_ASSOCIATESERVICEPARAMETERS * params, LST_HEAD ** list, unsigned long *rtnLength);
static CONDITION
...
static CONDITION
streamSubItem(DUL_SUBITEM * item, unsigned char *b, unsigned long *len);
static CONDITION
streamPresentationContext(LST_HEAD ** presentationContextList, unsigned char *b, unsigned long *length);
static CONDITION
streamUserInfo(DUL_USERINFO * userInfo, unsigned char *b, unsigned long *length);
static CONDITION
streamMaxLength(DUL_MAXLENGTH * max, unsigned char *b, unsigned long *length);
static CONDITION
streamSCUSCPList(LST_HEAD ** list, unsigned char *b, unsigned long *length);
static CONDITION
streamSCUSCPRole(PRV_SCUSCPROLE * scuscpRole, unsigned char *b, unsigned long *len);
...
CONDITION
constructAssociatePDU(DUL_ASSOCIATESERVICEPARAMETERS * params, unsigned char type, PRV_ASSOCIATEPDU * pdu)
{
    CONDITION		cond;			/* Variable used to examine condition returned by lower level routines. */
    unsigned long   itemLength;		/* Holds length of an item computed by a lower level routine. */
    DUL_PRESENTATIONCONTEXT		* presentationCtx;	/* Pointer to loop through presentation context items */
    PRV_PRESENTATIONCONTEXTITEM	* contextItem;		/* A presentation context item which is
    ...
    if (pdu->presentationContextList == NULL)
    	return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "constructAssociatePDU");
    ...
    if (strlen(params->calledAPTitle) < 1 || strlen(params->calledAPTitle) > 16)
    	return COND_PushCondition(DUL_ILLEGALSERVICEPARAMETER, DUL_Message(DUL_ILLEGALSERVICEPARAMETER), "Called AP Title");
	(void) strcpy(pdu->calledAPTitle, params->calledAPTitle);
    pdu->length += 16;
    if (strlen(params->callingAPTitle) < 1 || strlen(params->callingAPTitle) > 16)
    	return COND_PushCondition(DUL_ILLEGALSERVICEPARAMETER, DUL_Message(DUL_ILLEGALSERVICEPARAMETER), "Calling AP Title");
    (void) strcpy(pdu->callingAPTitle, params->callingAPTitle);
    pdu->length += 16;
    (void) memset(pdu->rsv3, 0, 32);
    pdu->length += 32;

    cond = constructSubItem(params->applicationContextName, DUL_TYPEAPPLICATIONCONTEXT, &pdu->applicationContext, &itemLength);
    ...
    if (type == DUL_TYPEASSOCIATERQ) {
    	if (debug) fprintf(DEBUG_DEVICE, "Constructing Associate RQ PDU\n");
    	presentationCtx = LST_Head(&params->requestedPresentationContext);
    	(void) LST_Position(&params->requestedPresentationContext, presentationCtx);
    	while (presentationCtx != NULL && cond == DUL_NORMAL) {
    		contextItem = (PRV_PRESENTATIONCONTEXTITEM *) CTN_MALLOC(sizeof(*contextItem));
    		if (contextItem == NULL)
    			return COND_PushCondition(DUL_MALLOCERROR,
										  DUL_Message(DUL_MALLOCERROR), "constructAssociatePDU",
										  sizeof(*contextItem));

			cond = constructPresentationContext(type, presentationCtx->presentationContextID,
												presentationCtx->result,
												presentationCtx->abstractSyntax,
												&presentationCtx->proposedTransferSyntax, NULL,
												contextItem, &itemLength);

			if (LST_Enqueue(&pdu->presentationContextList, contextItem) != LST_NORMAL)
				return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "constructAssociatePDU");

			pdu->length += itemLength;
			presentationCtx = LST_Next(&params->requestedPresentationContext);
    	}
    }else{
    	if (debug) fprintf(DEBUG_DEVICE, "Constructing Associate AC PDU\n");
    	if (params->acceptedPresentationContext != NULL) {
    		presentationCtx = LST_Head(&params->acceptedPresentationContext);
    		if (presentationCtx != NULL) (void) LST_Position(&params->acceptedPresentationContext, presentationCtx);
    		while (presentationCtx != NULL && cond == DUL_NORMAL) {
    			contextItem = (PRV_PRESENTATIONCONTEXTITEM *) CTN_MALLOC(sizeof(*contextItem));
    			if (contextItem == NULL)
    				return COND_PushCondition(DUL_MALLOCERROR,
...
CONDITION
constructAssociateRejectPDU(unsigned char result, unsigned char source, unsigned char reason, DUL_REJECTRELEASEABORTPDU * pdu)
{
...
*/
CONDITION
constructAbortPDU(unsigned char src, unsigned char reason, DUL_REJECTRELEASEABORTPDU * pdu)
{
...
CONDITION
streamAssociatePDU(PRV_ASSOCIATEPDU * assoc, unsigned char *b, unsigned long maxLength, unsigned long *rtnLen)
{
    ...
    *rtnLen += subLength;
    ...
    cond = streamPresentationContext(&assoc->presentationContextList, b, &subLength);
    if (cond != DUL_NORMAL)	return cond;
...
{
    if (maxLength < 2 + 2 + 4)
    	return COND_PushCondition(DUL_CODINGERROR, DUL_Message(DUL_CODINGERROR), "buffer too short in streamRejectReleaseAbortPDU");
...
CONDITION
streamDataPDUHead(DUL_DATAPDU * pdu, unsigned char *buf, unsigned long maxLength, unsigned long *rtnLen)
{
    unsigned long 		l;

#ifdef PDV_TEST
    if (maxLength < 18)
    	return COND_PushCondition(DUL_CODINGERROR, DUL_Message(DUL_CODINGERROR), "buffer too short in streamDataPDUHead");
#else
    if (maxLength < 12)
    	return COND_PushCondition(DUL_CODINGERROR, DUL_Message(DUL_CODINGERROR), "buffer too short in streamDataPDUHead");
#endif
...
*/
static CONDITION
constructSubItem(char *name, unsigned char type, DUL_SUBITEM * subItem, unsigned long *rtnLength)
{
    if (strlen(name) < 1 || strlen(name) > 64)
    	return COND_PushCondition(DUL_ILLEGALSERVICEPARAMETER, DUL_Message(DUL_ILLEGALSERVICEPARAMETER), name);
							 ...
							 PRV_PRESENTATIONCONTEXTITEM * context, unsigned long *rtnLen)
{
    CONDITION    		cond;
    unsigned long       length;
    DUL_SUBITEM			* subItem;		/* Subitem pointer created for transfer syntax items */
    DUL_TRANSFERSYNTAX	* transfer;		/* Pointer to loop through list of transfer syntaxes */
    ...
    *rtnLen += 8;
    ...
    if (associateType == DUL_TYPEASSOCIATERQ) {
    	cond = constructSubItem(abstractSyntax, DUL_TYPEABSTRACTSYNTAX, &(context->abstractSyntax), &length);
    	if (cond != DUL_NORMAL) return cond;
    ...
    if (context == NULL)
    	return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "constructPresentationContext");

    if (associateType == DUL_TYPEASSOCIATERQ) {
    	transfer = LST_Head(proposedTransferSyntax);
    	if (transfer == NULL)
    		return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "constructPresentationContext");
    	...
    	while (transfer != NULL) {
    		...
    		cond = constructSubItem(transfer->transferSyntax, DUL_TYPETRANSFERSYNTAX, subItem, &length);
    		if (cond != DUL_NORMAL) return cond;
    	...
    	cond = constructSubItem(acceptedTransferSyntax, DUL_TYPETRANSFERSYNTAX, subItem, &length);
    	if (cond != DUL_NORMAL) return cond;
...
*/
static CONDITION
constructUserInfo(unsigned char type, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_USERINFO * userInfo, unsigned long *rtnLen)
{
    CONDITION		   cond;
    unsigned long	   length;
...
constructSCUSCPRoles(unsigned char type,
					 DUL_ASSOCIATESERVICEPARAMETERS * params, LST_HEAD ** list, unsigned long *rtnLength)
{
    CONDITION				    cond;
    DUL_PRESENTATIONCONTEXT		* presentationCtx;	/* Pointer to loop through presentation ctx */
    ...
    if (type == DUL_TYPEASSOCIATERQ) {
    	...
    	while (presentationCtx != NULL) {
    		if (presentationCtx->proposedSCRole != DUL_SC_ROLE_DEFAULT) {
				...
				if (scuscpItem == NULL)
					return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "constructSCUSCPRoles", sizeof(*scuscpItem));
				...
				*rtnLength += length;
				...
				if (cond != LST_NORMAL)
					return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "constructSCUSCPRoles");
    		}
    		presentationCtx = LST_Next(&params->requestedPresentationContext);
    	}
    }else{
    	...
    	while (presentationCtx != NULL) {
    		if (presentationCtx->acceptedSCRole != DUL_SC_ROLE_DEFAULT) {
    			...
    			if (scuscpItem == NULL)
    				return COND_PushCondition(DUL_MALLOCERROR,
											  DUL_Message(DUL_MALLOCERROR), "constructSCUSCPRoles", sizeof(*scuscpItem));
				...
				cond = constructSCUSCPSubItem(presentationCtx->abstractSyntax, DUL_TYPESCUSCPROLE, scuRole, scpRole, scuscpItem, &length);
				if (cond != DUL_NORMAL) return cond;
...
static CONDITION
constructSCUSCPSubItem(char *name, unsigned char type, unsigned char scuRole,
					   unsigned char scpRole, PRV_SCUSCPROLE * scuscpItem, unsigned long *length)
{
    if (strlen(name) < 1 || strlen(name) > 64)
    	return COND_PushCondition(DUL_ILLEGALSERVICEPARAMETER, DUL_Message(DUL_ILLEGALSERVICEPARAMETER), name);
...
streamPresentationContext(LST_HEAD ** presentationContextList, unsigned char *b, unsigned long *length)
{
    ...
    *length = 0;
    ...
    if (presentation == NULL)
    	return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "streamPresentationContext");
    ...
    while (presentation != NULL) {
    	...
    	*length += 8;
    	...
    	if (transfer == NULL) return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "streamPresentationContext");
...
streamSCUSCPList(LST_HEAD ** list, unsigned char *b, unsigned long *length)
{
    ...
    *length = 0;
    ...
    if (scuscpRole != NULL)	(void) LST_Position(list, scuscpRole);
