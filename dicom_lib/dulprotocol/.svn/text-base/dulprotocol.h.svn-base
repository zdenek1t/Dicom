...
**			Layer) facility.
** Last Update:		$Author: smm $, $Date: 2002/07/23 20:31:45 $
...
typedef enum {
    ...
    DUL_DATASETPDV		/* A data set PDV inside a data PDU */
}	DUL_DATAPDV;


typedef enum {
    DUL_BLOCK,			/* Block on an operation (read, request) */
    DUL_NOBLOCK			/* Or return immediately if nothing avail */
}	DUL_BLOCKOPTIONS;
...
CONDITION
DUL_AcknowledgeAssociationRQ(DUL_ASSOCIATIONKEY ** association, DUL_ASSOCIATESERVICEPARAMETERS * params);
CONDITION
DUL_InitializeNetwork(char *type, char *mode, void *param, int timeout, unsigned long options, DUL_NETWORKKEY ** network);
CONDITION
DUL_ReceiveAssociationRQ(DUL_NETWORKKEY ** net, DUL_BLOCKOPTIONS blk, DUL_ASSOCIATESERVICEPARAMETERS * parameters, DUL_ASSOCIATIONKEY ** association);
CONDITION
...
CONDITION
DUL_RejectAssociationRQ(DUL_ASSOCIATIONKEY ** association, DUL_ABORTITEMS * params);
CONDITION
DUL_RequestAssociation(DUL_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_ASSOCIATIONKEY ** association);
CONDITION
DUL_RequestAssociationWithProxy(DUL_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_PROXY* proxy, DUL_ASSOCIATIONKEY ** association);
CONDITION
...
*/
CONDITION
DUL_AbortAssociation(DUL_ASSOCIATIONKEY ** association);
CONDITION
DUL_DropAssociation(DUL_ASSOCIATIONKEY ** association);
CONDITION
DUL_DropNetwork(DUL_NETWORKKEY ** network);
CONDITION
DUL_ReleaseAssociation(DUL_ASSOCIATIONKEY ** association);
CONDITION
DUL_AcknowledgeRelease(DUL_ASSOCIATIONKEY ** association);

/* Functions for reading/write PDVs inside P DATA PDUs.
*/
CONDITION
DUL_ReadPDVs(DUL_ASSOCIATIONKEY ** association, DUL_PDVLIST * pdvList, DUL_BLOCKOPTIONS block, int timeout);
CONDITION
DUL_WritePDVs(DUL_ASSOCIATIONKEY ** association, DUL_PDVLIST * pdvList);
CONDITION
DUL_NextPDV(DUL_ASSOCIATIONKEY ** association, DUL_PDV * pdv);
...
CONDITION
DUL_AssociationParameter(DUL_ASSOCIATIONKEY ** association, DUL_ASSOCIATION_PARAMETER param, DUL_DATA_TYPE type, void *address, size_t length);
CONDITION
DUL_MakePresentationCtx(DUL_PRESENTATIONCONTEXT ** ctx, DUL_SC_ROLE proposedSCRole, DUL_SC_ROLE acceptedSCRole,
						DUL_PRESENTATIONCONTEXTID ctxID, unsigned char result, const char *abstractSyntax, const char *transferSyntax,...);
CONDITION
DUL_AddSinglePresentationCtx(DUL_ASSOCIATESERVICEPARAMETERS* params, DUL_SC_ROLE proposedRole, DUL_SC_ROLE acceptedRole,
			DUL_PRESENTATIONCONTEXTID contextID, unsigned char result, const char* abstractSyntax, const char** xferSyntaxes, int xferSyntaxCount);
CONDITION
DUL_AddMultiplePresentationCtx(DUL_ASSOCIATESERVICEPARAMETERS* params, DUL_SC_ROLE proposedRole, DUL_SC_ROLE acceptedRole,
			DUL_PRESENTATIONCONTEXTID contextID, unsigned char result, const char* abstractSyntax, const char** xferSyntaxes, int xferSyntaxCount);
