...
static CONDITION
parseSubItem(DUL_SUBITEM * subItem, unsigned char *buf, unsigned long *itemLength);
static CONDITION
parsePresentationContext(unsigned char type, PRV_PRESENTATIONCONTEXTITEM * context, unsigned char *buf, unsigned long *itemLength);
static CONDITION
parseUserInfo(DUL_USERINFO * userInfo, unsigned char *buf, unsigned long *itemLength);
static CONDITION
parseMaxPDU(DUL_MAXLENGTH * max, unsigned char *buf, unsigned long *itemLength);
static CONDITION
parseDummy(unsigned char *buf, unsigned long *itemLength);
static CONDITION
parseSCUSCPRole(PRV_SCUSCPROLE * role, unsigned char *buf, unsigned long *length);
static void
...
CONDITION
parseAssociate(unsigned char *buf, unsigned long pduLength, PRV_ASSOCIATEPDU * assoc)
{
    CONDITION						cond;
    unsigned char			        type;
    ...
    if ((assoc->presentationContextList = LST_Create()) == NULL)
    	return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "parseAssociate");
    ...
    if ((assoc->protocol & DUL_PROTOCOL) == 0)
    	return COND_PushCondition(DUL_UNSUPPORTEDPEERPROTOCOL, DUL_Message(DUL_UNSUPPORTEDPEERPROTOCOL), assoc->protocol, DUL_PROTOCOL, "parseAssociate");
...
*/
		(void) fprintf(DEBUG_DEVICE, "PDU type: %lx (%s), PDU Length: %ld\n",(long) assoc->type, s, assoc->length);
...
#ifdef DEBUG
    	if (debug) {
    		(void) fprintf(DEBUG_DEVICE, "=============== Parsing remaining %ld bytes of A-ASSOCIATE PDU ===============\n", pduLength);
    		(void) fprintf(DEBUG_DEVICE, "Next item type: %02lx\n", (unsigned long) type);
    	}
#endif
    	switch (type) {
...
#ifdef DEBUG
												if (debug)(void) fprintf(DEBUG_DEVICE, "Successfully parsed Application Context\n");
#endif
											}
											break;
			...
			case DUL_TYPEPRESENTATIONCONTEXTAC:
											...
											if (context == NULL)
												return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "parseAssociate", sizeof(*context));
											...
											buf += itemLength;
											pduLength -= itemLength;
											if (LST_Enqueue(&assoc->presentationContextList, context) != LST_NORMAL)
												return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "parseAssociate");
#ifdef DEBUG
											if (debug)(void) fprintf(DEBUG_DEVICE, "Successfully parsed Presentation Context\n");
#endif
											break;
			case DUL_TYPEUSERINFO:
											...
											pduLength -= itemLength;
#ifdef DEBUG
											if (debug)(void) fprintf(DEBUG_DEVICE, "Successfully parsed User Information\n");
#endif
											break;
			default:
											...
											buf += itemLength;
											pduLength -= itemLength;
											if (debug)(void) fprintf(DEBUG_DEVICE, "ILLEGAL PDU! %02lx\n", (unsigned long) assoc->type);
											/*cond = COND_PushCondition(DUL_ILLEGALPDU, DUL_Message(DUL_ILLEGALPDU), (unsigned long) assoc->type); */
											break;
    	}
    }
    return cond;
...
#ifdef DEBUG
    if (debug) fprintf(DEBUG_DEVICE, "Subitem parse: Type %02lx, Length %4ld, Content: %s\n", (unsigned long) subItem->type, (unsigned long) subItem->length, subItem->data);
#endif
    return DUL_NORMAL;
}
...
parsePresentationContext(unsigned char type, PRV_PRESENTATIONCONTEXTITEM * context, unsigned char *buf, unsigned long *itemLength)
{
    ...
    if ((context->transferSyntaxList = LST_Create()) == NULL)
    	return COND_PushCondition(DUL_LISTCREATEFAILED, DUL_Message(DUL_LISTCREATEFAILED), "parsePresentationContext");
...
#ifdef DEBUG
    if (debug) {
    	(void) fprintf(DEBUG_DEVICE,"=============== Parsing Presentation Context: (%2lx), Length: %ld ===============\n",(unsigned long) context->type, (unsigned long) context->length);
    	(void) fprintf(DEBUG_DEVICE, "Presentation Context ID: %2lx\n",(unsigned long) context->contextID);
    }
#endif
...
#ifdef DEBUG
    		if (debug) {
    			(void) fprintf(DEBUG_DEVICE, "=============== Parsing remaining %ld bytes of Presentation Ctx ===============\n", presentationLength);
    			(void) fprintf(DEBUG_DEVICE, "Next item type: %02lx\n", (unsigned long) *buf);
    		}
#endif
	    switch (*buf) {
...
#ifdef DEBUG
											if (debug) (void) fprintf(DEBUG_DEVICE, "Successfully parsed Abstract Syntax\n");
#endif
											break;
			case DUL_TYPETRANSFERSYNTAX:
											subItem = CTN_MALLOC(sizeof(*subItem));
											if (subItem == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "parsePresentationContext", sizeof(*subItem));

											cond = parseSubItem(subItem, buf, &length);
											if (cond != DUL_NORMAL) return cond;

											if (LST_Enqueue(&context->transferSyntaxList, subItem) != LST_NORMAL) return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "parsePresentationContext");

											buf += length;
											presentationLength -= length;
#ifdef DEBUG
											if (debug)(void) fprintf(DEBUG_DEVICE, "Successfully parsed Transfer Syntax\n");
#endif
											break;
			default:
											...
											presentationLength -= length;
											if (debug)(void) fprintf(DEBUG_DEVICE, "ILLEGAL PDU! %02lx\n", (unsigned long) *buf);
											/* return COND_PushCondition(DUL_ILLEGALPDU, DUL_Message(DUL_ILLEGALPDU), 0xfff);*/
											break;
			}
    	}
    }
...
#ifdef DEBUG
    if (debug) (void) fprintf(DEBUG_DEVICE, "Parsing user info field (%02lx), Length: %ld\n", (unsigned long) userInfo->type, (unsigned long) userInfo->length);
#endif
    while (userLength > 0) {
#ifdef DEBUG
	if (debug) {
	    (void) fprintf(DEBUG_DEVICE, "=============== Parsing remaining %d bytes of User Information ===============\n", userLength);
	    (void) fprintf(DEBUG_DEVICE, "Next item type: %02lx\n", (unsigned long) *buf);
	}
#endif
	switch (*buf) {
...
#ifdef DEBUG
								if (debug) (void) fprintf(DEBUG_DEVICE, "Successfully parsed Maximum PDU Length\n");
#endif
								break;
		...
		case DUL_TYPESCUSCPROLE:
								...
								if (role == NULL) return COND_PushCondition(DUL_MALLOCERROR, DUL_Message(DUL_MALLOCERROR), "parseUserInfo", sizeof(*role));

								cond = parseSCUSCPRole(role, buf, &length);
								if (cond != DUL_NORMAL)	return cond;

								cond = LST_Enqueue(&userInfo->SCUSCPRoleList, role);
								if (cond != LST_NORMAL)	return COND_PushCondition(DUL_LISTERROR, DUL_Message(DUL_LISTERROR), "parseUserInfo");
...
#ifdef DEBUG
    if (debug) (void) fprintf(DEBUG_DEVICE, "Maximum PDU Length: %ld\n", (unsigned long) max->maxLength);
#endif
...
/*
static CONDITION
parseImplementationClassUID(PRV_IMPLEMENTATIONCLASSUID *p, unsigned char *buf, unsigned long *itemLength)
{
    p->type = *buf++;
    p->rsv1 = *buf++;
...
#ifdef DEBUG
    if (debug) {
    	fprintf(DEBUG_DEVICE, "Subitem parse: Type %02lx, Length %4ld, Content: %s\
		%d %d \n ",	(unsigned long) role->type, (unsigned long) role->length, role->SOPClassUID, (int) role->SCURole, (int) role->SCPRole);
    }
#endif
    return DUL_NORMAL;
}
