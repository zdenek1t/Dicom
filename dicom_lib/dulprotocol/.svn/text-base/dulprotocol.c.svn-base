...
static CONDITION
createNetworkKey(char *type, char *mode, int timeout, unsigned long opt, PRIVATE_NETWORKKEY ** k);
static CONDITION
createAssociationKey(PRIVATE_NETWORKKEY ** net, char *node, unsigned long maxPDU, PRIVATE_ASSOCIATIONKEY ** assoc);
static CONDITION
initializeNetworkTCP(PRIVATE_NETWORKKEY ** k, void *p);
static CONDITION
receiveTransportConnection(PRIVATE_NETWORKKEY ** network, DUL_BLOCKOPTIONS block, DUL_ASSOCIATESERVICEPARAMETERS * params, CTN_SOCKET* s);
static CONDITION
receiveTransportConnectionTCP(PRIVATE_NETWORKKEY ** network, DUL_BLOCKOPTIONS block, DUL_ASSOCIATESERVICEPARAMETERS * params, CTN_SOCKET* s);
static void
destroyAssociationKey(PRIVATE_ASSOCIATIONKEY ** key);
static CONDITION
get_association_parameter(void *paramAddress, DUL_DATA_TYPE paramType, size_t paramLength, DUL_DATA_TYPE outputType, void *outputAddress, size_t outputLength);
static void
setTCPBufferLength(int sock);
static CONDITION
checkNetwork(PRIVATE_NETWORKKEY ** networkKey, char *caller);
static CONDITION
checkAssociation(PRIVATE_ASSOCIATIONKEY ** association, char *caller);
static void
...
static CONDITION
requestAssociationTCP(PRIVATE_NETWORKKEY ** network, DUL_ASSOCIATESERVICEPARAMETERS * params, CTN_SOCKET* s)
{
  char 					node[128];
  int 					port;
  ...
  if (sscanf(params->calledPresentationAddress, "%[^:]:%d", node, &port) != 2)
	  return COND_PushCondition(DUL_ILLEGALSERVICEPARAMETER, DUL_Message(DUL_ILLEGALSERVICEPARAMETER), params->calledPresentationAddress);
...
CONDITION
DUL_InitializeNetwork(char *networkType, char *mode, void *networkParameter, int timeout, unsigned long opt, DUL_NETWORKKEY ** networkKey)
{
    PRIVATE_NETWORKKEY		* key;			/* Our local copy of the key which is created */
    CONDITION				cond;
#ifdef _MSC_VER
    WSADATA 				wsaData;
#endif
...
#ifdef DEBUG
    if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_InitializeNetwork, Type: %s, Mode: %s\n", networkType, mode);
#endif
	...
	setsockopt(INVALID_SOCKET, SOL_SOCKET, SO_OPENTYPE, (char *) &nOption, sizeof(int));
    }
#endif
    ...
    *networkKey = NULL;
#if 0
    if (networkInitialized++) return COND_PushCondition(DUL_NETWORKINITIALIZED, DUL_Message(DUL_NETWORKINITIALIZED));
#endif
...
{
    PRIVATE_NETWORKKEY		** networkKey;
    CONDITION				cond;

#ifdef DEBUG
    if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_DropNetwork\n");
#endif
...
CONDITION
DUL_RequestAssociation(DUL_NETWORKKEY ** callerNetworkKey, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_ASSOCIATIONKEY ** callerAssociation)
{
  DUL_PROXY 			proxy;
  CTN_SOCKET* 			s = 0;
  ...
  if (((*network)->applicationFunction & PRV_APPLICATION_REQUESTOR) == 0) return COND_PushCondition(DUL_ILLEGALREQUEST, DUL_Message(DUL_ILLEGALREQUEST));

  s = (CTN_SOCKET*) CTN_MALLOC(sizeof(*s));
  if (s == NULL) return COND_PushCondition(DUL_MALLOCERROR, "DUL_RequestAssociation", sizeof(*s));
  ...
  cond = DUL_RequestAssociationWithProxy(callerNetworkKey, params, &proxy, callerAssociation);
  return cond;
}
...
#if 0
{
...
#if 0
    cond = PRV_StateMachine(network, association, A_ASSOCIATE_REQ_LOCAL_USER, (*network)->protocolState, params);
#endif
    ...
    if (cond != DUL_NORMAL) {
    	 ...
    	 * state
    	 */
    	cond = PRV_StateMachine(network, association, TRANS_CONN_CLOSED, (*association)->protocolState, NULL);
    	destroyAssociationKey(association);
    	return COND_PushCondition(DUL_REQUESTASSOCIATIONFAILED, DUL_Message(DUL_REQUESTASSOCIATIONFAILED));
    }
    cond = PRV_StateMachine(network, association, TRANS_CONN_CONFIRM_LOCAL_USER, (*association)->protocolState, params);
    if (cond != DUL_NORMAL) {
    	destroyAssociationKey(association);
    	return cond;
    }
...
CONDITION
DUL_RequestAssociationWithProxy(DUL_NETWORKKEY ** callerNetworkKey,	DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_PROXY* proxy, DUL_ASSOCIATIONKEY ** callerAssociation)
{
  ...
  if (((*network)->applicationFunction & PRV_APPLICATION_REQUESTOR) == 0)
	  return COND_PushCondition(DUL_ILLEGALREQUEST, DUL_Message(DUL_ILLEGALREQUEST));

  if (params->maxPDU < MIN_PDU_LENGTH || params->maxPDU > MAX_PDU_LENGTH)
	  return COND_PushCondition(DUL_ILLEGALPARAMETER, DUL_Message(DUL_ILLEGALPARAMETER), "maxPDU", "DUL_RequestAssociation");
  ...
  /* send a request primitive */
  if (cond != DUL_NORMAL) {
	  /* In case of an error, close the connection and go to the initial state */
	  cond = PRV_StateMachine(network, association, TRANS_CONN_CLOSED, (*association)->protocolState, NULL);
	  destroyAssociationKey(association);
	  return COND_PushCondition(DUL_REQUESTASSOCIATIONFAILED, DUL_Message(DUL_REQUESTASSOCIATIONFAILED));
  }
...
CONDITION
DUL_ReqAssocExternalSocket(DUL_NETWORKKEY ** callerNetworkKey, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_ASSOCIATIONKEY ** callerAssociation, CTN_SOCKET sock)
{
    PRIVATE_ASSOCIATIONKEY	** association;
    PRIVATE_NETWORKKEY		** network;
    CONDITION				cond;
...
#ifdef DEBUG
    if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_Request Association \n");
#endif
...
CONDITION
DUL_ReceiveAssociationRQ(DUL_NETWORKKEY ** callerNetworkKey, DUL_BLOCKOPTIONS block, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_ASSOCIATIONKEY ** callerAssociation)
{
    PRIVATE_NETWORKKEY		** network;
    PRIVATE_ASSOCIATIONKEY	** association;
    CONDITION				cond;
...
/*    unsigned char			pduType;
    ...
    if (((*network)->applicationFunction & PRV_APPLICATION_ACCEPTOR) == 0)
    	return COND_PushCondition(DUL_ILLEGALACCEPT, DUL_Message(DUL_ILLEGALACCEPT));

    if (params->maxPDU < MIN_PDU_LENGTH || params->maxPDU > MAX_PDU_LENGTH)
    	return COND_PushCondition(DUL_ILLEGALPARAMETER, DUL_Message(DUL_ILLEGALPARAMETER), "maxPDU", "DUL_RequestAssociation");

    s = (CTN_SOCKET*) CTN_MALLOC(sizeof(*s));
    if (s == NULL) return COND_PushCondition(DUL_MALLOCERROR, "DUL_ReceiveAssociationRequest", sizeof(*s));

	#ifdef DEBUG
    	if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_Receive Association RQ\n");
	#endif
    ...
    cond = DUL_ReceiveAssociationRQWithProxy(callerNetworkKey, block, params, &proxy, callerAssociation);
  ...
  (*association)->networkState = NETWORK_CONNECTED;
    ...
    cond = PRV_StateMachine(network, association, TRANS_CONN_INDICATION, (*network)->protocolState, params);
    if (cond != DUL_NORMAL)	return cond;
...
/*		(void) DUL_RejectAssociationRQ(callerAssociation, &abortItems); */
    	(void) PRV_StateMachine(NULL, association, A_ASSOCIATE_RESPONSE_REJECT, (*association)->protocolState, &abortItems);
    }
    if (networkClosed) return DUL_NETWORKCLOSED;
    return cond;
#endif
}

CONDITION
DUL_ReceiveAssociationRQWithProxy(DUL_NETWORKKEY ** callerNetworkKey, DUL_BLOCKOPTIONS block, DUL_ASSOCIATESERVICEPARAMETERS * params, DUL_PROXY* proxy, DUL_ASSOCIATIONKEY ** callerAssociation)
{
  PRIVATE_NETWORKKEY 		** network;
  PRIVATE_ASSOCIATIONKEY 	** association;
  CONDITION 				cond;
  ...
  if (*network == 0) {
	  DUL_InitializeFSM();
	  cond = createNetworkKey(DUL_NETWORK_TCP, DUL_AEBOTH, DUL_TIMEOUT, DUL_ORDERBIGENDIAN, network);
	  if (!CTN_SUCCESS(cond)) return cond;
  }
  ...
  if (params->maxPDU < MIN_PDU_LENGTH || params->maxPDU > MAX_PDU_LENGTH)
	  return COND_PushCondition(DUL_ILLEGALPARAMETER, DUL_Message(DUL_ILLEGALPARAMETER), "maxPDU", "DUL_RequestAssociationWithProxy");

  cond = createAssociationKey(network, params->callingPresentationAddress, params->maxPDU, association);
  if (cond != DUL_NORMAL) return cond;
...
CONDITION
DUL_AcknowledgeAssociationRQ(DUL_ASSOCIATIONKEY ** callerAssociation, DUL_ASSOCIATESERVICEPARAMETERS * params)
{
    PRIVATE_ASSOCIATIONKEY		** association;
    CONDITION					cond;
...
#ifdef DEBUG
    if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_Acknowledge Association RQ\n");
#endif
    ...
    if (blog) {
    	...
    	BLG_Write(&(*association)->logHandle, type, (*association)->fragmentBuffer, (*association)->nextPDULength);
    }
#endif

    cond = PRV_StateMachine(NULL, association, A_ASSOCIATE_RESPONSE_ACCEPT, (*association)->protocolState, params);
...
CONDITION
DUL_RejectAssociationRQ(DUL_ASSOCIATIONKEY ** callerAssociation, DUL_ABORTITEMS * params)
{
    PRIVATE_ASSOCIATIONKEY			** association;
    CONDITION						cond;
    DUL_ABORTITEMS					localParams;
...
#ifdef DEBUG
    if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_Reject Association RQ\n");
#endif
    ...
    localParams.source = 0x01;
    {
    	...
    	if (!found) return COND_PushCondition(DUL_ILLEGALREJECTREASON, DUL_Message(DUL_ILLEGALREJECTREASON), localParams.reason);
    }
    {
    	...
    	if (!found) return COND_PushCondition(DUL_ILLEGALREJECTRESULT, DUL_Message(DUL_ILLEGALREJECTRESULT), localParams.result);
    }

    cond = PRV_StateMachine(NULL, association, A_ASSOCIATE_RESPONSE_REJECT, (*association)->protocolState, &localParams);
    if (!CTN_SUCCESS(cond))	return cond;

    cond = PRV_StateMachine(NULL, association, ARTIM_TIMER_EXPIRED, (*association)->protocolState, NULL);
...
#ifdef DEBUG
    if (debug)(void) fprintf(DEBUG_DEVICE, "DUL_DropAssociation\n");
#endif
...
DUL_ReleaseAssociation(DUL_ASSOCIATIONKEY ** callerAssociation)
{
    ...
    cond = PRV_StateMachine(NULL, association, A_RELEASE_REQ, (*association)->protocolState, NULL);
    if (cond != DUL_NORMAL)	return cond;

    cond = PRV_NextPDUType(association, DUL_NOBLOCK, PRV_DEFAULTTIMEOUT, &pduType);
    if (cond == DUL_NETWORKCLOSED){
    	event = TRANS_CONN_CLOSED;
    	if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = Network closed \n");
    }else if (cond == DUL_READTIMEOUT){
    	event = ARTIM_TIMER_EXPIRED;
    	if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = Timer expired \n");
    }else if (cond != DUL_NORMAL){
    	if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: non-normal condition %08lx \n", cond);
    	return cond;
    }else{
    	switch (pduType) {
			case DUL_TYPEASSOCIATERQ:
										event = A_ASSOCIATE_RQ_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_ASSOCIATE_RQ_PDU_RCV \n");
										break;
			case DUL_TYPEASSOCIATEAC:
										event = A_ASSOCIATE_AC_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_ASSOCIATE_AC_PDU_RCV \n");
										break;
			case DUL_TYPEASSOCIATERJ:
										event = A_ASSOCIATE_RJ_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_ASSOCIATE_RJ_PDU_RCV \n");
										break;
			case DUL_TYPEDATA:
										event = P_DATA_TF_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = P_DATA_TF_PDU_RCV \n");
										break;
			case DUL_TYPERELEASERQ:
										event = A_RELEASE_RQ_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_RELEASE_RQ_PDU_RCV\n");
										break;
			case DUL_TYPERELEASERP:
										event = A_RELEASE_RP_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_RELEASE_RP_PDU_RCV\n");
										break;
			case DUL_TYPEABORT:
										event = A_ABORT_PDU_RCV;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_ABORT_PDU_RCV\n");
										break;
			default:
										event = INVALID_PDU;
										if (debug) fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: event = A_ASSOCIATE_RQ_PDU_RCV \n");
    ...
    if (debug){
    	fprintf(DEBUG_DEVICE, "DUL_ReleaseAssociation:: final condition = %08ld\n", cond);
    	COND_PushCondition(cond, DUL_Message(cond));
    }
...
DUL_AcknowledgeRelease(DUL_ASSOCIATIONKEY ** callerAssociation)
{
    ...
    cond = PRV_StateMachine(NULL, association, A_RELEASE_RESP, (*association)->protocolState, NULL);
    if (!CTN_SUCCESS(cond))	return cond;
...
CONDITION
DUL_WritePDVs(DUL_ASSOCIATIONKEY ** callerAssociation, DUL_PDVLIST * pdvList)
{
    PRIVATE_ASSOCIATIONKEY		** association;
    CONDITION					cond;
    ...
    cond = PRV_StateMachine(NULL, association, P_DATA_REQ, (*association)->protocolState, pdvList);
...
CONDITION
DUL_ReadPDVs(DUL_ASSOCIATIONKEY ** callerAssociation, DUL_PDVLIST * pdvList, DUL_BLOCKOPTIONS block, int timeout)
{
    PRIVATE_ASSOCIATIONKEY		** association;
    CONDITION					cond;
    unsigned char				pduType;
    ...
    cond = PRV_StateMachine(NULL, association, event, (*association)->protocolState, pdvList);
...
CONDITION
DUL_AssociationParameter(DUL_ASSOCIATIONKEY ** callerAssociation, DUL_ASSOCIATION_PARAMETER param, DUL_DATA_TYPE type, void *address, size_t length)
{
    PRIVATE_ASSOCIATIONKEY		** association;
    CONDITION					cond;
		...
		case DUL_K_MAX_PDV_XMIT:
								/*	if (strcmp((*association)->applicationType, AE_ACCEPTOR) == 0) { */
									if ((*association)->applicationFunction == PRV_APPLICATION_ACCEPTOR) {
										cond = get_association_parameter( &((*association)->maxPDVRequestor),
																		  DUL_K_INTEGER,
																		  sizeof((*association)->maxPDVRequestor),
																		  type, address, length);
									}else{
										cond = get_association_parameter( &((*association)->maxPDVAcceptor),
																		 DUL_K_INTEGER,
																		 sizeof((*association)->maxPDVAcceptor),
																		 type, address, length);
									}
									if (cond != DUL_NORMAL) cond = COND_PushCondition(DUL_ASSOCIATIONPARAMETERFAILED,
...
DUL_NextPDV(DUL_ASSOCIATIONKEY ** callerAssociation, DUL_PDV * pdv)
{
    ...
    if ((*association)->pdvIndex == -1)	return COND_PushCondition(DUL_NOPDVS, DUL_Message(DUL_NOPDVS));
...
receiveTransportConnection(PRIVATE_NETWORKKEY ** network, DUL_BLOCKOPTIONS block, DUL_ASSOCIATESERVICEPARAMETERS * params, CTN_SOCKET* s)
{
    if (strcmp((*network)->networkType, DUL_NETWORK_TCP) == 0){
    	return receiveTransportConnectionTCP(network, block, params, s);
    }else{
...
receiveTransportConnectionTCP(PRIVATE_NETWORKKEY ** network, DUL_BLOCKOPTIONS block, DUL_ASSOCIATESERVICEPARAMETERS * params, CTN_SOCKET* s)
{
    ...
    if (block == DUL_NOBLOCK) {
    	...
    	timeout.tv_sec = 0;
    	timeout.tv_usec = 0;
    	nfound = select((*network)->networkSpecific.TCP.listenSocket + 1, &fdset, NULL, NULL, &timeout);
    	if (nfound != 0) if (FD_ISSET((*network)->networkSpecific.TCP.listenSocket, &fdset)) connected++;
    	if (!connected) return COND_PushCondition(DUL_NOASSOCIATIONREQUEST, DUL_Message(DUL_NOASSOCIATIONREQUEST));
    }else{
    	connected = 0;
    	do {
    		FD_ZERO(&fdset);
    ...
    *s = -1;
...
#endif
    	if (debug) fprintf(DEBUG_DEVICE, "************* AFTER ACCEPT(sock: %d/errno: %d - %s) *************\n\n\n", *s, errno, strerror(errno));

    } while (*s == -1 && errno == EINTR);

    if (*s < 0){
    	(void) COND_PushCondition(DUL_TCPINITERROR, "Accept Error In %s, socket %d", "receiveTransportConnectionTCP", *s);
    ...
    if (setsockopt(*s, SOL_SOCKET, SO_LINGER, (char *) &sockarg, sizeof(sockarg)) < 0) {
    	(void) COND_PushCondition(DUL_TCPINITERROR, "In %s, socket %d", "receiveTransportConnectionTCP", *s);
    	return COND_PushCondition(DUL_TCPINITERROR, strerror(errno));
    }
    reuse = 1;
...
#ifdef CTN_USE_THREADS
#ifdef SOLARIS
    remote = gethostbyaddr_r(&from.sa_data[2], 4, 2, &hStorage, hBuf, sizeof(hBuf), &hError);
#else
    ...
    if (remote == NULL) {
    	...
    	if ((*network)->options & DUL_FULLDOMAINNAME){
    		...
    		if (sscanf(remote->h_name, "%[^.]", node) != 1) node[0] = '\0';
    	}
...
*/
static CONDITION
createNetworkKey(char *networkType, char *mode, int timeout, unsigned long opt, PRIVATE_NETWORKKEY ** key)
{
    if (strcmp(networkType, DUL_NETWORK_TCP) != 0) return COND_PushCondition(DUL_UNSUPPORTEDNETWORK, DUL_Message(DUL_UNSUPPORTEDNETWORK), networkType);
    if (strcmp(mode, AE_REQUESTOR) != 0 && strcmp(mode, AE_ACCEPTOR) != 0 && strcmp(mode, AE_BOTH) != 0) return COND_PushCondition(DUL_UNRECOGNIZEDAE, DUL_Message(DUL_UNRECOGNIZEDAE), mode);

    *key = (DUL_NETWORKKEY *) CTN_MALLOC(sizeof(PRIVATE_NETWORKKEY));
    if (*key == NULL) return COND_PushCondition(DUL_KEYCREATEFAILURE, DUL_Message(DUL_KEYCREATEFAILURE));
    ...
    if (strcmp(mode, AE_REQUESTOR) == 0){
    	...
    	(*key)->applicationFunction |= PRV_APPLICATION_ACCEPTOR | PRV_APPLICATION_REQUESTOR;
    }
...
initializeNetworkTCP(PRIVATE_NETWORKKEY ** key, void *parameter)
{
    ...
    if ((*key)->applicationFunction & PRV_APPLICATION_ACCEPTOR) {
		...
		reuse = 1;
		if (setsockopt((*key)->networkSpecific.TCP.listenSocket, SOL_SOCKET, SO_REUSEADDR, (char *) &reuse, sizeof(reuse)) < 0)
			return COND_PushCondition(DUL_TCPINITERROR, DUL_Message(DUL_TCPINITERROR), strerror(errno));
		...
		if (bind((*key)->networkSpecific.TCP.listenSocket, (struct sockaddr *) & server, sizeof(server)))
			return COND_PushCondition(DUL_TCPINITERROR, DUL_Message(DUL_TCPINITERROR), strerror(errno));

		/* Find out assigned port number and print it out */
		length = sizeof(server);
		if (getsockname((*key)->networkSpecific.TCP.listenSocket, (struct sockaddr *) & server, &length))
			return COND_PushCondition(DUL_TCPINITERROR, DUL_Message(DUL_TCPINITERROR), strerror(errno));

		sockarg.l_onoff = 0;
		if (setsockopt((*key)->networkSpecific.TCP.listenSocket, SOL_SOCKET, SO_LINGER, (char *) &sockarg, sizeof(sockarg)) < 0)
			return COND_PushCondition(DUL_TCPINITERROR, DUL_Message(DUL_TCPINITERROR), strerror(errno));

		if (debug) fprintf(DEBUG_DEVICE, "\n\n\n***BEFORE LISTEN***\n");
		listen((*key)->networkSpecific.TCP.listenSocket, PRV_LISTENBACKLOG);
		if (debug) fprintf(DEBUG_DEVICE, "***AFTER LISTEN***\n\n\n");
    }
    return DUL_NORMAL;
}
...
*/
static CONDITION
createAssociationKey(PRIVATE_NETWORKKEY ** networkKey, char *remoteNode, unsigned long maxPDU, PRIVATE_ASSOCIATIONKEY ** associationKey)
{
    PRIVATE_ASSOCIATIONKEY		* key;


    key = (PRIVATE_ASSOCIATIONKEY *) CTN_MALLOC( sizeof(PRIVATE_ASSOCIATIONKEY) + maxPDU + 100);
    if (key == NULL) return COND_PushCondition(DUL_KEYCREATEFAILURE, DUL_Message(DUL_KEYCREATEFAILURE));
...
get_association_parameter(void *paramAddress, DUL_DATA_TYPE paramType, size_t paramLength, DUL_DATA_TYPE outputType, void *outputAddress, size_t outputLength)
{
    if (paramType != outputType)
    	return COND_PushCondition(DUL_WRONGDATATYPE, DUL_Message(DUL_WRONGDATATYPE));
    if ((paramType == DUL_K_INTEGER) && (outputLength != paramLength))
    	return COND_PushCondition(DUL_INCORRECTBUFFERLENGTH, DUL_Message(DUL_INCORRECTBUFFERLENGTH));
    if ((paramType == DUL_K_STRING) && (outputLength < strlen(paramAddress)))
    	return COND_PushCondition(DUL_INSUFFICIENTBUFFERLENGTH, DUL_Message(DUL_INSUFFICIENTBUFFERLENGTH));
...
dump_presentation_ctx(LST_HEAD ** l)
{
    ...
    while (ctx != NULL) {
    	...
    	for (index = 0; index < (int) DIM_OF(scMap); index++) {
    		if (ctx->proposedSCRole == scMap[index].role) printf("  Proposed SCU/SCP Role:  %s\n", scMap[index].text);
    	}
    	for (index = 0; index < (int) DIM_OF(scMap); index++) {
    		if (ctx->acceptedSCRole == scMap[index].role) printf("  Accepted SCU/SCP Role:  %s\n", scMap[index].text);
    	}
    	printf("  Proposed Xfer Syntax(es)\n");
    	if (ctx->proposedTransferSyntax != NULL) {
    		transfer = LST_Head(&ctx->proposedTransferSyntax);
    		if (transfer != NULL)(void) LST_Position(&ctx->proposedTransferSyntax, transfer);
    	...
    	ctx = LST_Next(l);
        printf("-------------- END Dump Presentation ctx --------------\n");
    }
...
checkNetwork(PRIVATE_NETWORKKEY ** networkKey, char *caller)
{
    if (networkKey == NULL)	 return COND_PushCondition(DUL_NULLKEY, DUL_Message(DUL_NULLKEY), caller);
    if (*networkKey == NULL) return COND_PushCondition(DUL_NULLKEY, DUL_Message(DUL_NULLKEY), caller);
    if (strcmp((*networkKey)->keyType, KEY_NETWORK) != 0) return COND_PushCondition(DUL_ILLEGALKEY, DUL_Message(DUL_ILLEGALKEY), caller);
...
checkAssociation(PRIVATE_ASSOCIATIONKEY ** association, char *caller)
{
    if (association == NULL)  return COND_PushCondition(DUL_NULLKEY, DUL_Message(DUL_NULLKEY), caller);
    if (*association == NULL) return COND_PushCondition(DUL_NULLKEY, DUL_Message(DUL_NULLKEY), caller);
    if (strcmp((*association)->keyType, KEY_ASSOCIATION) != 0) return COND_PushCondition(DUL_ILLEGALKEY, DUL_Message(DUL_ILLEGALKEY), caller);
