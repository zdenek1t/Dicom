...
*/
static CONDITION
newElementItem(DCM_ELEMENT * src, CTNBOOLEAN allocateData, PRV_ELEMENT_ITEM ** dst);
static CONDITION
findCreateGroup(PRIVATE_OBJECT ** object, unsigned short group, PRV_GROUP_ITEM ** groupPtr);
static CONDITION
insertNewElement(PRIVATE_OBJECT ** object, DCM_ELEMENT * element);
static CONDITION
updateObjectType(PRIVATE_OBJECT ** object, DCM_ELEMENT * element);
static CONDITION
updateSpecialElements(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * item);
static void
exportFixedFields(DCM_ELEMENT * element, unsigned char *b, U32 length, int byteOrder, CTNBOOLEAN explicitVR, U32 * rtnLength);
static CONDITION
exportData(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * item, unsigned char *src, unsigned char *dst, U32 length, int byteOrder, U32 * rtnLength);
...
static CONDITION
countBytes(void *buffer, U32 length, int flag, void /* unsigned long */ *sizePtr);
static CONDITION
exportStream(DCM_OBJECT ** callerObject, unsigned long opt, void *buffer, U32 bufferlength, CONDITION(*callback) (), void *ctx, int sequenceLevel);
static CONDITION
verifyFormat(PRV_ELEMENT_ITEM * item);

#if 0
static CONDITION
readFile(char *name, unsigned char *callerBuf, int fd, long size, off_t fileOffset, int recursionLevel, unsigned long opt, DCM_OBJECT ** callerObject,
		 U32 * scannedLength, CTNBOOLEAN * remainOpenFlag, void *ctx, CONDITION(*rd) (void *ctx, void *buf, int toRead, int *bytesRead),
		 CONDITION(*sk) (void *ctx, int offset, int flag));
#endif
static CONDITION
readFile1(const char *name, unsigned char *callerBuf, int fd, U32 size, off_t * fileOffset, int recursionLevel, unsigned long opt, PRIVATE_OBJECT ** parentObject,
		  DCM_OBJECT ** callerObject, U32 * scannedLength, CTNBOOLEAN * remainOpenFlag, void *ctx, CONDITION(*rd) (void *ctx, void *buf, int toRead, int *bytesRead),
		  CONDITION(*sk) (void *ctx, int offset, int flag));
...
static CONDITION
copyData(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * item, DCM_ELEMENT * to, U32 * rtnLength);
static CONDITION
readLengthToEnd(int fd, const char *fileName, unsigned long opt, U32 * lengthToEnd);
#ifdef LITTLE_ENDIAN_ARCHITECTURE
static void
swapATGroupElement(DCM_ELEMENT * e);
#endif
static void
dumpBinaryData(void *d, DCM_VALUEREPRESENTATION vr, long vm, long vmLimit);
static void
compareGroup(PRV_GROUP_ITEM * g1, PRV_GROUP_ITEM * g2, void (*callback) (const DCM_ELEMENT * e1, const DCM_ELEMENT * e2, void *ctx), void *ctx);
static void
...
DCM_OpenFile(const char *name, unsigned long opt, DCM_OBJECT ** callerObject)
{
    ...
    U32 		lengthToEnd;
    U32 		size;
    CTNBOOLEAN	remainFileOpen = FALSE;	/* Leave file open after parse ? */
    ...
    if (fd < 0) return COND_PushCondition(DCM_FILEOPENFAILED, DCM_Message(DCM_FILEOPENFAILED), name, "DCM_OpenFile");

    size = fileSize(fd);
    if (size <= 0) return DCM_FILEACCESSERROR;
...
#endif
    cond = readFile1(name, NULL, fd, size, &fileOffset, 0, opt, NULL, callerObject, NULL, &remainFileOpen, NULL, NULL, NULL);
    if ((cond != DCM_NORMAL) || !remainFileOpen)
    	(void) close(fd);
		if (cond != DCM_NORMAL) {
			if (debug) DCM_DumpElements(callerObject, 1);
			return COND_PushCondition(DCM_FILEOPENFAILED, DCM_Message(DCM_FILEOPENFAILED), name, "DCM_OpenFile");
		}else{
			return DCM_NORMAL;
		}
...
CONDITION
DCM_ReadStream(DCM_OBJECT ** callerObject, unsigned long opt, long size, void *ctx,
	  CONDITION(*rd) (void *ctx, void *buf, int toRead, int *bytesRead), CONDITION(*sk) (void *ctx, int offset, int flag))
{
    CONDITION 		cond;
    int 			fd = -1;
    CTNBOOLEAN		remainFileOpen = FALSE;	/* Leave file open after parse ? */
...
DCM_CreateObject(DCM_OBJECT ** object, unsigned long opt)
{
    ...
    if (obj == NULL) {
    	(void) COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(PRIVATE_OBJECT), "DCM_CreateObject");
    	*object = NULL;
    	return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_CreateObject");
    }
...
{
    CONDITION			cond;
    PRV_GROUP_ITEM		* group;
    PRV_ELEMENT_ITEM	* element;
    PRIVATE_OBJECT		** object;
    DCM_SEQUENCE_ITEM	* sequenceItem;
    DCM_FRAGMENT_ITEM* 	fragmentItem;

    if (debug) fprintf(DEBUG_DEVICE, "Starting DCM_CloseObject\n");
    ...
    if ((*object)->fd > 0) (void) close((*object)->fd);

    if (debug) fprintf(DEBUG_DEVICE, "DCM_CloseObject: Legal object and file closed\n");

    while ((group = LST_Pop(&(*object)->groupList)) != NULL) {
    	if (debug) fprintf(DEBUG_DEVICE, "DCM_CloseObject: group %04x\n", group->group);
    	while ((element = LST_Pop(&group->elementList)) != NULL) {
    		if (debug) fprintf(DEBUG_DEVICE, "DCM_CloseObject: Element %08ld\n", element->element.tag);
    		if (element->element.representation == DCM_SQ) {
    			if (debug) fprintf(DEBUG_DEVICE, "Sequence List Address: %p\n", element->element.d.sq);
    			...
    			if (debug) fprintf(DEBUG_DEVICE, "Fragment List Address: %p\n", element->element.d.fragments);
    		...
    		if (debug) fprintf(stderr, "DCM_CloseObject: free %p\n", element);

	    CTN_FREE(element);
    	}
    	cond = LST_Destroy(&group->elementList);
    	if (cond != LST_NORMAL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_CloseObject");
    	CTN_FREE(group);
    }
...
/****   	(void) COND_PushCondition(DCM_FILEDELETEFAILED, strerror(errno));****/
    		cond = COND_PushCondition(DCM_FILEDELETEFAILED, DCM_Message(DCM_FILEDELETEFAILED), (*object)->fileName, strerror(errno), "DCM_CloseObject");
...
DCM_AddElement(DCM_OBJECT ** callerObject, DCM_ELEMENT * element)
{
    ...
    if ((DCM_TAG_ELEMENT(element->tag) == 0x0000))
    	return COND_PushCondition(DCM_ILLEGALADD, DCM_Message(DCM_ILLEGALADD), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_AddElement");
    ...
    if (cond != DCM_NORMAL) {
    	...
    	if (element->representation != DCM_UN && element->representation != localElement.representation) {
    		return COND_PushCondition(DCM_ILLEGALREPRESENTATION, DCM_Message(DCM_ILLEGALREPRESENTATION),
									  DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_AddElement");
    	}
    }

    cond = findCreateGroup(object, DCM_TAG_GROUP(localElement.tag), &groupItem);
    if (cond != DCM_NORMAL)
    	return COND_PushCondition(DCM_INSERTFAILED, DCM_Message(DCM_INSERTFAILED), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_AddElement");
...
DCM_AddSequenceElement(DCM_OBJECT ** callerObject, DCM_ELEMENT * element)
{
    ...
    if (localElement.representation != DCM_SQ)
    	return COND_PushCondition(DCM_NOTASEQUENCE, DCM_Message(DCM_NOTASEQUENCE), DCM_TAG_GROUP(localElement.tag), DCM_TAG_ELEMENT(localElement.tag), "DCM_AddSequenceElement");

    cond = findCreateGroup(object, DCM_TAG_GROUP(localElement.tag), &groupItem);
    if (cond != DCM_NORMAL)
    	return COND_PushCondition(DCM_INSERTFAILED, DCM_Message(DCM_INSERTFAILED), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_AddSequenceElement");
...
DCM_RemoveElement(DCM_OBJECT ** callerObject, DCM_TAG tag)
{
    PRIVATE_OBJECT		** object;
    PRV_GROUP_ITEM		* groupItem;
    PRV_ELEMENT_ITEM	* elementItem, *groupLengthItem;
    CONDITION			cond;
    CTNBOOLEAN			flag;
    ...
    if (groupItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), group, element, "DCM_RemoveElement");
    ...
    if (DCM_TAG_ELEMENT(groupLengthItem->element.tag) != 0x0000) groupLengthItem = NULL;
    ...
    if (groupItem->baseLength != DCM_UNSPECIFIEDLENGTH) {
    	...
    	if (groupLengthItem != NULL) *groupLengthItem->element.d.ul = groupItem->baseLength;
    }
    if ((*object)->objectSize != DCM_UNSPECIFIEDLENGTH)	(*object)->objectSize -= elementItem->paddedDataLength + 2 + 2 + 4;

    if (elementItem->element.representation == DCM_OW || elementItem->element.representation == DCM_OB || elementItem->element.representation == DCM_SQ) {
...
DCM_GetElementValue(DCM_OBJECT ** callerObject, DCM_ELEMENT * element, U32 * rtnLength, void **ctx)
{
    ...
    groupItem = LST_Head(&(*object)->groupList);
    if (groupItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_GetElementValue");

    (void) LST_Position(&(*object)->groupList, groupItem);
    while (groupItem != NULL) {
    	if (groupItem->group == DCM_TAG_GROUP(element->tag)) break;
    	groupItem = LST_Next(&(*object)->groupList);
    }
    if (groupItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_GetElementValue");
    ...
    elementItem = LST_Head(&groupItem->elementList);
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(element->tag), DCM_TAG_GROUP(element->tag), "DCM_GetElementValue");
    ...
    while (elementItem != NULL) {
    	if (elementItem->element.tag == element->tag) {
    		...
    		if (element->representation == DCM_SQ)
    			return COND_PushCondition(DCM_CANNOTGETSEQUENCEVALUE, DCM_Message(DCM_CANNOTGETSEQUENCEVALUE), element->tag, "DCM_GetElementValue");

    		p = *ctx;
    		if ((U32) p > elementItem->element.length) return COND_PushCondition(DCM_ILLEGALCONTEXT, DCM_Message(DCM_ILLEGALCONTEXT), "DCM_GetElementValue");
    		...
    		*rtnLength = l;
    		{
    			if (elementItem->element.d.ot == NULL) {
    				if ((*object)->fd != -1) {
    					(void) lseek((*object)->fd,	elementItem->dataOffset + (off_t) p, SEEK_SET);
    					nBytes = read((*object)->fd, element->d.ot, (int) l);
    				}else{
    					(*object)->sk((*object)->userCtx, (long) (elementItem->dataOffset + (off_t) p), SEEK_SET);
    					cond = (*object)->rd((*object)->userCtx, element->d.ot, l, &nBytes);
    				}
    				if ((unsigned) nBytes != l)
    					return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "DCM_GetElementValue");
    				...
    				unsigned char *q;
    				q = (unsigned char *) elementItem->element.d.ot + (U32) p;
    			...
    			p += l;
    			*ctx = (void *) p;
/* ZT - nekorektni delka z CT */
    			if ( (unsigned) p == elementItem->element.length){
    				return DCM_NORMAL;
    			}else{
    				fprintf(stderr,"Element %08lx has not correct length %d <=> %ld\n", elementItem->element.tag, (unsigned)p, elementItem->element.length);
    			}
    			return DCM_NORMAL;
    			//return DCM_GETINCOMPLETE;
    		}
...
DCM_GetString(DCM_OBJECT** callerObject, DCM_TAG tag)
{
  ...
  if (e.representation == DCM_SQ) return 0;
  if (e.length > sizeof(b)) return 0;
...
DCM_GetElementValueOffset(DCM_OBJECT ** callerObject, DCM_ELEMENT * element, unsigned long offset)
{
    ...
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_GetElementValueOffset");
	...
	if (element->representation == DCM_SQ)
	    return COND_PushCondition(DCM_CANNOTGETSEQUENCEVALUE, DCM_Message(DCM_CANNOTGETSEQUENCEVALUE), element->tag, "DCM_GetElementValueOffset");

	p = (unsigned char *) offset;;
	if ((U32) p > elementItem->element.length)
	    return COND_PushCondition(DCM_BADOFFSET, DCM_Message(DCM_BADOFFSET), (int) offset, (int) elementItem->element.length, "DCM_GetElementValueLength");

	l = element->length;
	if (l + offset > elementItem->element.length)
	    return COND_PushCondition(DCM_BADLENGTH, DCM_Message(DCM_BADLENGTH), (int) offset, (int) l, (int) elementItem->element.length, "DCM_GetElementValueLength");

	{
	    if (elementItem->element.d.ot == NULL) {
	    	...
	    	if ((unsigned) nBytes != l)
	    		return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "DCM_GetElementValueValue");
...
DCM_GetElementSize(DCM_OBJECT ** callerObject, DCM_TAG tag, U32 * rtnLength)
{
    ...
    groupItem = LST_Head(&((*object)->groupList));
    if (groupItem == NULL) return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), group, element, "DCM_GetElementSize");
    ...
    if (flag == FALSE) return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), group, element, "DCM_GetElementSize");

    elementItem = LST_Head(&groupItem->elementList);
    if (elementItem == NULL) return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), group, element, "DCM_GetElementSize");
...
CONDITION
DCM_ScanParseObject(DCM_OBJECT ** callerObject, void *buf, size_t bufferSize, DCM_FLAGGED_ELEMENT * elementVector, int vectorLength,
...
{
#ifdef DEBUG
    if (debug) (void) fprintf(DEBUG_DEVICE, "DCM_ImportStream, %ld bytes\n", length);
#endif

    if ((opt & DCM_ORDERMASK) == 0)	return COND_PushCondition(DCM_ILLEGALOPTION, DCM_Message(DCM_ILLEGALOPTION), "Byte order", "DCM_ImportStream");
...
CONDITION
DCM_ExportStream(DCM_OBJECT ** callerObject, unsigned long opt, void *buffer, unsigned long bufferlength, DCM_EXPORT_STREAM_CALLBACK* callback, void *ctx)
{
...
DCM_DumpElements(DCM_OBJECT ** callerObject, long vm)
{
    ...
    if (groupItem != NULL) (void) LST_Position(&(*object)->groupList, groupItem);
...
#endif
	elementItem = LST_Head(&groupItem->elementList);
	if (elementItem != NULL) (void) LST_Position(&groupItem->elementList, elementItem);

	while (elementItem != NULL) {
#ifdef MACOS
		(void) printf("%04x %04x %8ld ", DCM_TAG_GROUP(elementItem->element.tag), DCM_TAG_ELEMENT(elementItem->element.tag), elementItem->element.length);
#else
	    (void) printf("%04x, %04x  length: %8ld ", DCM_TAG_GROUP(elementItem->element.tag), DCM_TAG_ELEMENT(elementItem->element.tag), elementItem->element.length);
#endif
	    ...
	    if (elementItem->element.d.ot == NULL){
	    	...
	    	switch (elementItem->element.representation) {
...
#else
								(void) printf("%8lx %ld\n", *elementItem->element.d.sl, *elementItem->element.d.sl);
								if (vm > 1)	dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length / sizeof(U32), vm);
#endif
								break;
				case DCM_SS:
								(void) printf("%4x %d\n", *elementItem->element.d.ss, *elementItem->element.d.ss);
								if (vm > 1)	dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length / sizeof(short), vm);
								break;
				case DCM_SQ:
								(void) printf("SEQUENCE\n");
								sq = LST_Head(&elementItem->element.d.sq);
								if (sq != NULL) (void) LST_Position(&elementItem->element.d.sq, sq);
				...
				case DCM_US:
								...
								if (vm > 1)	dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length / sizeof(unsigned short), vm);
								break;
				case DCM_OB:
				case DCM_UN:
								dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length , 8);
								break;
...
DCM_FormatElements(DCM_OBJECT ** callerObject, long vm, const char* prefix)
{
    ...
    while (groupItem != NULL) {
    	printf("%sGroup: %04x, Length: %8ld\n", prefix, groupItem->group, groupItem->baseLength);
    	elementItem = LST_Head(&groupItem->elementList);
    	if (elementItem != NULL) (void) LST_Position(&groupItem->elementList, elementItem);
    	while (elementItem != NULL) {
    		(void) printf("%s%04x %04x %8ld ", prefix, DCM_TAG_GROUP(elementItem->element.tag), DCM_TAG_ELEMENT(elementItem->element.tag), elementItem->element.length);
    		(void) printf("//%31s//", elementItem->element.description);
    		if (elementItem->element.d.ot == NULL){
    			(void) printf("Data on disk\n");
    		}else{
    			switch (elementItem->element.representation) {
					...
					case DCM_SL:
									...
									(void) printf("%8lx %ld\n", *elementItem->element.d.sl, *elementItem->element.d.sl);
									if (vm > 1)
										dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length / sizeof(U32), vm);
#endif
									break;
					case DCM_SS:
									(void) printf("%4x %d\n", *elementItem->element.d.ss, *elementItem->element.d.ss);
									if (vm > 1)
										dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length / sizeof(short), vm);
									break;
					case DCM_SQ:
									(void) printf("SEQUENCE\n");
									sq = LST_Head(&elementItem->element.d.sq);
					...
					case DCM_US:
									(void) printf("%4x %d\n", *elementItem->element.d.us, *elementItem->element.d.us);
									if (vm > 1)
										dumpBinaryData(elementItem->element.d.ot, elementItem->element.representation, elementItem->element.length / sizeof(unsigned short), vm);
									break;
					case DCM_OT:
					case DCM_OW:
					case DCM_OB:
...
#endif
    if (fd < 0)	return COND_PushCondition(DCM_FILECREATEFAILED, DCM_Message(DCM_FILECREATEFAILED), file, strerror(errno), "DCM_WriteFile");
...
CONDITION
DCM_ModifyElements(DCM_OBJECT ** callerObject, DCM_ELEMENT * vector, int count, DCM_FLAGGED_ELEMENT * flaggedVector, int flaggedCount, int *updateCount)
{
    PRIVATE_OBJECT		** object;
    CONDITION			cond;
    DCM_ELEMENT			e;
    ...
    while (count-- > 0) {
    	...
    	if (cond != DCM_NORMAL) (void) COND_PopCondition(FALSE);

    	e = *vector;
    	if (DCM_IsString(e.representation)) e.length = strlen(e.d.string);
    ...
    if (updateCount != NULL) *updateCount = c;
    return DCM_NORMAL;
}
...
*/
CONDITION
DCM_ParseObject(DCM_OBJECT ** callerObject, DCM_ELEMENT * vector, int count, DCM_FLAGGED_ELEMENT * flaggedVector, int flagCount, int *parseCount)
{
    PRIVATE_OBJECT		** object;
    CONDITION			cond;
    void		        *ctx;
    ...
    if (parseCount != NULL)	*parseCount = c;
    return DCM_NORMAL;
}
...
DCM_RemoveGroup(DCM_OBJECT ** callerObject, unsigned short group)
{
    ...
    groupItem = LST_Head(&(*object)->groupList);
    if (groupItem == NULL) return COND_PushCondition(DCM_GROUPNOTFOUND, DCM_Message(DCM_GROUPNOTFOUND), (int) group, "DCM_RemoveGroup");
    ...
    if (cond != LST_NORMAL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_RemoveGroup");
...
DCM_GetSequenceList(DCM_OBJECT ** object, DCM_TAG tag, LST_HEAD ** list)
{
    ...
    if (groupItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetSequenceList");

    (void) LST_Position(&(*obj)->groupList, groupItem);

    while (groupItem != NULL) {
    	if (groupItem->group == DCM_TAG_GROUP(tag)) break;
    ...
    elementItem = LST_Head(&groupItem->elementList);
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag), DCM_TAG_GROUP(tag), "DCM_GetSequenceTag");
...
DCM_GetSequenceElement(DCM_OBJECT ** object, DCM_TAG top, DCM_ELEMENT * e)
{
    ...
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(top), DCM_TAG_ELEMENT(top), "DCM_GetElementSequence");

    if (elementItem->element.representation != DCM_SQ)
    	return COND_PushCondition(DCM_UNEXPECTEDREPRESENTATION, DCM_Message(DCM_UNEXPECTEDREPRESENTATION), "DCM_GetSequenceElement", "sequence");
...
DCM_GetElementValueList(DCM_OBJECT ** object, DCM_TAG tag, size_t structureSize, long stringOffset, LST_HEAD ** list)
{
    ...
    if (!found)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetElementValueList");

    if (!DCM_IsString(elementItem->element.representation))
    	return COND_PushCondition(DCM_UNEXPECTEDREPRESENTATION, DCM_Message(DCM_UNEXPECTEDREPRESENTATION), "DCM_GetElementValueList", "string");
    ...
    while (l > 0) {
    	...
    	if ((l == 1) && (*src == ' ' || *src == DCM_DELIMITOR)) l--;

    	if (l != 0) {
    		p = CTN_MALLOC(structureSize);
    		if (p == NULL) return COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), structureSize, "DCM_GetElementValueList");
    		dst = p + stringOffset;
    		...
    		*dst = '\0';;
    		...
    		if (cond != LST_NORMAL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_GetElementValueList");
    	}
    }
    return DCM_NORMAL;
}
...
{
    DCM_ELEMENT			e;			/* Local copy of caller's element */
    CONDITION			cond;
    char		        *s;
...
DCM_GetElement(DCM_OBJECT ** callerObject, DCM_TAG tag, DCM_ELEMENT * element)
{
    ...
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_GetElementVM");
...
CONDITION
DCM_CompareAttributes(DCM_OBJECT ** o1, DCM_OBJECT ** o2, void (*callback) (const DCM_ELEMENT * e1, const DCM_ELEMENT * e2, void *ctx), void *ctx)
{
    ...
    if (groupItem1 != NULL)	(void) LST_Position(&(*object1)->groupList, groupItem1);

    groupItem2 = LST_Head(&(*object2)->groupList);
    if (groupItem2 != NULL)	(void) LST_Position(&(*object2)->groupList, groupItem2);
...
DCM_GroupPresent(DCM_OBJECT ** o1, U16 group)
{
    ...
    if (item != NULL) (void) LST_Position(&(*object)->groupList, item);
...
newElementItem(DCM_ELEMENT * src, CTNBOOLEAN allocateData, PRV_ELEMENT_ITEM ** dst)
{
    ...
    if (debug) fprintf(DEBUG_DEVICE, "newElementItem: CTN_MALLOC %8ld %8ld ", l, sizeof(PRV_ELEMENT_ITEM) + l);

    *dst = (PRV_ELEMENT_ITEM *) CTN_MALLOC(sizeof(PRV_ELEMENT_ITEM) + l);

    if (debug) fprintf(DEBUG_DEVICE, "%p\n", *dst);

    if (*dst == NULL)
    	return COND_PushCondition(DCM_ELEMENTCREATEFAILED, DCM_Message(DCM_ELEMENTCREATEFAILED), "newElementItem", DCM_TAG_GROUP(src->tag), DCM_TAG_ELEMENT(src->tag), l);
...
static CONDITION
findCreateGroup(PRIVATE_OBJECT ** object, unsigned short group,	PRV_GROUP_ITEM ** groupItem)
{
    PRV_GROUP_ITEM    * item;
    CONDITION		  cond;
    CTNBOOLEAN		  tooFar = FALSE;
	...
	if (newGroupItem == NULL)
	    return COND_PushCondition(DCM_ELEMENTCREATEFAILED, DCM_Message(DCM_ELEMENTCREATEFAILED), "findCreateGroup", group, 0xffff, sizeof(*newGroupItem));
...
insertNewElement(PRIVATE_OBJECT ** object, DCM_ELEMENT * element)
{
    PRV_ELEMENT_ITEM	* nextItem, *newItem;
    PRV_GROUP_ITEM		* groupItem;
    CONDITION			cond;
    char		        *p;
...
/*	    	newItem->element.length = 0xffffffff; */
    		...
    		CTN_FREE(newItem);
    		return COND_PushCondition(DCM_UNEVENELEMENTLENGTH, DCM_Message(DCM_UNEVENELEMENTLENGTH),
									  DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), element->length, "insertNewElement");
    	}
    }else if (newItem->element.representation != DCM_SQ){
...
/*		newItem->element.length = 0xffffffff; */
    ...
    if ((*object)->objectSize != DCM_UNSPECIFIEDLENGTH) (*object)->objectSize += 8 + newItem->paddedDataLength;
...
/* repair */
    ...
    if (groupItem == NULL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "insertNewElement");

    if (groupItem->baseLength != DCM_UNSPECIFIEDLENGTH)	groupItem->baseLength += 2 + 2 + 4 + newItem->paddedDataLength;

    if (newItem->element.representation == DCM_OW || newItem->element.representation == DCM_OB || newItem->element.representation == DCM_SQ) {
    	groupItem->longVRAttributes++;
    	(*object)->longVRAttributes++;
    }
...
*/
    ...
    while (nextItem != NULL) {
    	if (DCM_TAG_GROUP(element->tag) != DCM_TAG_GROUP(nextItem->element.tag)) {
    		return COND_PushCondition(DCM_BADELEMENTINGROUP, DCM_Message(DCM_BADELEMENTINGROUP), DCM_TAG_GROUP(nextItem->element.tag),
									  DCM_TAG_ELEMENT(nextItem->element.tag), groupItem->group, "insertNewElement");
...
/* repair */
  ...
  if (groupItem == NULL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "insertThisElementItem");
...
*/
  ...
  while (nextItem != NULL) {
	  if (DCM_TAG_GROUP(newItem->element.tag) != DCM_TAG_GROUP(nextItem->element.tag)) {
		  return COND_PushCondition(DCM_BADELEMENTINGROUP, DCM_Message(DCM_BADELEMENTINGROUP),
									DCM_TAG_GROUP(nextItem->element.tag), DCM_TAG_ELEMENT(nextItem->element.tag), groupItem->group, "insertThisElementItem");
...
updateObjectType(PRIVATE_OBJECT ** object, DCM_ELEMENT * element)
{
    switch ((*object)->objectType) {
		case DCM_OBJECTUNKNOWN:
									...
									break;
		case DCM_OBJECTCOMMAND:
									if (DCM_TAG_GROUP(element->tag) != DCM_GROUPCOMMAND) (*object)->objectType = DCM_OBJECTELEMENTLIST;
									break;
		case DCM_OBJECTELEMENTLIST:
		case DCM_OBJECTIMAGE:
									break;
...
lookupVRCode(const char *code)
{
    ...
    for (i = 0; i < (int) DIM_OF(vrMap); i++) {
    	if (strcmp(code, vrMap[i].code) == 0) return &vrMap[i];
    }
    return NULL;
}
...
static void
exportVRLength(DCM_ELEMENT * e, unsigned char *b, int byteOrder, U32 * rtnLength)
{
    int 						i;
    char 						*c = "xx";
    unsigned char 				*p;
    ...
    vr = e->representation;
    if (e->tag == DCM_MAKETAG(0x003a, 0x1000)) vr = DCM_OB;
...
static CONDITION
exportPreamble(PRIVATE_OBJECT ** obj, unsigned char *dst, U32 bufferLength, U32 * rtnLength)
{
    *rtnLength = 0;
    if (bufferLength < (DCM_PREAMBLELENGTH + 4))
    	return COND_PushCondition(DCM_EXPORTBUFFERTOOSMALL, DCM_Message(DCM_EXPORTBUFFERTOOSMALL), (int) bufferLength, "exportPreamble");
...
exportFixedFields(DCM_ELEMENT * e, unsigned char *b, U32 length, int byteOrder, CTNBOOLEAN explicitVR, U32 * rtnLength)
{
    ...
    if (e->representation == DCM_DLM) explicitVR = FALSE;

    minimumLength = sizeof(group) + sizeof(element) + sizeof(e->length);
    if (explicitVR)	minimumLength += 4;
...
static CONDITION
exportData(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * item, unsigned char *src, unsigned char *b, U32 length, int byteOrder, U32 * rtnLength)
{
    ...
    *rtnLength = 0;
    ...
    if (element->d.ot == NULL) {
    	...
    	if ((U32) nBytes != length) {
    		...
    		return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "exportData");
    	}
#ifdef LITTLE_ENDIAN_ARCHITECTURE
    	...
    	*rtnLength = (U32) nBytes;
    	...
    	switch (element->representation) {
			...
			case DCM_US:
			case DCM_OW:
							/* Temorary hack by Nilesh to support memory mapping for testing purposes. */
							length &= ~1;
							*rtnLength += length;
...
exportEncapsulatedPixels(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * item,
						 unsigned char* buffer, U32 bufferlength, DCM_EXPORT_STREAM_CALLBACK* callback, void* ctx)
{
  DCM_ELEMENT 			* element;
  int 					nBytes;
  CONDITION 			cond;
  ...
  if (element->d.ot == NULL) {
	  if ((*object)->fd != -1) {
		  /* Seek to the beginning of the data. Have to back up 12 bytes to get the pixel tag, VR, etc  */
	  ...
	  while(toExport > 0) {
		  ...
		  if ((U32) nBytes != length) {
			  ...
			  return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "exportEncapsualtedPixels");
		  }
	  ...
	  if (item->fragmentFlag != 1)
		  return COND_PushCondition(DCM_NOFRAGMENTSINOBJECT, "DCM Exporting pixels but did not find expected fragments in object");
...
static CONDITION
exportPixels(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * item,	int encapsulatedPixels, unsigned char* buffer, U32 bufferlength, DCM_EXPORT_STREAM_CALLBACK* callback,
			 void* ctx,	int byteOrder, int explicitVR)
{
  DCM_ELEMENT 		* element;
  CONDITION 		cond;
  U32 				bytesExported = 0;
  ...
  if (encapsulatedPixels) return exportEncapsulatedPixels(object, item, buffer, bufferlength, callback, ctx);
  ...
  while (remainingData > 0) {
    if (debug) fprintf(DEBUG_DEVICE, "Export: (%08lx) %ld\n", element->tag, element->length);
    ...
    if (c <= 20) {
    	...
    	if (cond != DCM_NORMAL) return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportPixels");
  ...
  if (element->d.ot == NULL) {
	  ...
	  while(toExport > 0) {
		  ...
		  if ((U32) nBytes != length) {
			  char b[512];
			  sprintf(b, "byte count: %d %d, errno: %d", nBytes, length, errno);
			  (void) COND_PushCondition(DCM_GENERALWARNING, DCM_Message(DCM_GENERALWARNING), "exportPixels", b);
			  return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "exportPixels");
		  }
		  cond = callback(buffer, length, 0, ctx);
		  if (cond != DCM_NORMAL)
			  return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportStream");
...
swapInPlace(PRIVATE_OBJECT ** object, DCM_ELEMENT * e)
{
    ...
    length = e->length;
    p1 = e->d.ot;
    if (e->representation == DCM_US || e->representation == DCM_SS || e->representation == DCM_OW || e->representation == DCM_AT) {
    	if (e->tag == DCM_PXLPIXELDATA && (*object)->pixelBitsAllocated != 16) return;
...
checkObject(PRIVATE_OBJECT ** object, char *caller)
{
    if (object == NULL)	 return COND_PushCondition(DCM_NULLOBJECT, DCM_Message(DCM_NULLOBJECT), caller);
    if (*object == NULL) return COND_PushCondition(DCM_NULLOBJECT, DCM_Message(DCM_NULLOBJECT), caller);
    if (strcmp((*object)->keyType, KEY_DCM_OBJECT) != 0) return COND_PushCondition(DCM_ILLEGALOBJECT, DCM_Message(DCM_ILLEGALOBJECT), caller);
...
static CONDITION
writeFile(void *buffer, U32 length, int flag, void /* int */ *fdPtr)
{
    int         bytesWritten;
    int 		*fd;
...
static CONDITION
extractFileOptions(unsigned long opt, CTNBOOLEAN * part10File, CTNBOOLEAN * explicitVR, int *byteOrder, CTNBOOLEAN* encapsulatedPixels)
{
...
static U32
computeGroupLength(PRV_GROUP_ITEM * groupItem, CTNBOOLEAN explicitVR)
{
    return (explicitVR) ?
    groupItem->baseLength + 4 * groupItem->longVRAttributes : groupItem->baseLength;
...
static CONDITION
exportStream(DCM_OBJECT ** callerObject, unsigned long opt, void *buffer, U32 bufferlength, DCM_EXPORT_STREAM_CALLBACK* callback, void *ctx, int sequenceLevel)
{
    PRIVATE_OBJECT		** object;
    PRV_GROUP_ITEM		* groupItem;
    PRV_ELEMENT_ITEM	* elementItem;
    ...
    CTNBOOLEAN			unspecifiedSQLength = FALSE, explicitVR = FALSE, part10File = FALSE, encapsulatedPixels = FALSE;
...
/*  End of something that is out of place */
    if ((opt & DCM_SEQUENCELENGTHMASK) == DCM_UNSPECIFIEDLENGTHFLAG) unspecifiedSQLength = TRUE;
    ...
    if (sequenceLevel != 0) {
    	if (!unspecifiedSQLength) itemMarker.length = (*object)->objectSize;
...
#if 0
    if (groupItem == NULL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "exportStream");
#endif
    if (groupItem != NULL) (void) LST_Position(&(*object)->groupList, groupItem);
    	...
    	if (elementItem != NULL) {
    		...
    		if (DCM_TAG_ELEMENT(elementItem->element.tag) == 0x0000) {
    			...
    			*elementItem->element.d.ul = l;
    			 ...
    			 */
    			if (groupItem->group != 0x0000 && groupItem->group != 0x0002) elementItem = LST_Next(&groupItem->elementList);
    		}
    	}
    	while (elementItem != NULL) {
    		if (c <= 20) {
    		...
    		if (element.tag == DCM_PXLPIXELDATA) {
    			/* Lots of special rules for pixel data. Handle separately. First, dump the current buffer */
    			cond = callback(buffer, bytesExported, 0, ctx);
    			if (cond != DCM_NORMAL)
    				return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportStream");

    			cond = exportPixels(object, elementItem, encapsulatedPixels, buffer, bufferlength, callback, ctx, byteOrder, explicitVR);
    			if (cond != DCM_NORMAL) return cond;
    			...
    			if (unspecifiedSQLength) element.length = DCM_UNSPECIFIEDLENGTH;
    		...
    		if (element.tag == DCM_PXLPIXELDATA) {
    			...
    			if (element.d.sq != NULL) {
    				...
    				if (sequenceItem != NULL) (void) LST_Position(&element.d.sq, sequenceItem);
    			...
    			while (remainingData > 0) {
    				...
    				if (c <= 20) {
    					cond = callback(buffer, bytesExported, 0, ctx);
    					if (cond != DCM_NORMAL)	return COND_PushCondition(DCM_CALLBACKABORTED, DCM_Message(DCM_CALLBACKABORTED), "exportStream");
    ...
    if (element->length > 0) {
    	switch (element->representation) {
			...
			case DCM_UT:
							...
							/* Right now, dst points to the char follows the last char in the string. */
							stopFlag = FALSE;
							l = (int) element->length;
							p = dst - 1;
			...
			case DCM_PN:
							...
							break;
			case DCM_UI:
							if (element->d.string[element->length - 1] == '\0') element->length--;
		 ...
		 CONDITION(*sk) (void *ctx, int offset, int flag))
{
    ...
    U32		 	 		sequenceLength, localLength;
    PRIVATE_OBJECT		** object;
    PRV_GROUP_ITEM		* groupItem = NULL;
    unsigned short  	group, element, tagGroup, tagElement;
    DCM_ELEMENT			e, tagE;
    CTNBOOLEAN			pixelFlag, convertFlag = FALSE,	done = FALSE, knownLength = TRUE, sequenceDone = FALSE,	createGroupFlag, explicitVR = FALSE;
    unsigned char   	buf[8], *ptr;
    int		        	nBytes;
    PRV_ELEMENT_ITEM	* elementItem = NULL;
    DCM_OBJECT			* sequenceObject;
    DCM_SEQUENCE_ITEM	* sequenceItem;
    ...
    if ((opt & DCM_CONVERTMASK) == DCM_FORMATCONVERSION) convertFlag = TRUE;

    if (scannedLength != NULL) *scannedLength = 0;

    cond = DCM_CreateObject(callerObject, opt);
    if (cond != DCM_NORMAL)	return cond;

    object = (PRIVATE_OBJECT **) callerObject;
    if (fileFlag) strcpy((*object)->fileName, name);

    (*object)->fd = -1;
    (*object)->rd = rd;
    (*object)->sk = sk;
    (*object)->userCtx = ctx;
    if (size == (long) DCM_UNSPECIFIEDLENGTH) knownLength = FALSE;
    if ((fileFlag) && ((opt & DCM_DELETEMASK) == DCM_DELETEONCLOSE) && (recursionLevel == 0)) (*object)->deleteFlag = TRUE;
    if (knownLength && (size == 0))	done = TRUE;

    while (!done) {
    	if ((size < 8) && knownLength) {
    		if (debug) (void) DCM_DumpElements(callerObject, 0);
    	...
    	if (fileFlag) {
    		...
    		if (nBytes != 4) return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), name, "readFile");
    		ptr = buf;
    	}
    	if (knownLength) size -= 4;
		fileOffset += (off_t) 4;
		if (scannedLength != NULL) (*scannedLength) += 4;
		(*object)->objectSize += 4;
		...
		if (explicitVR) {
			...
			if (scannedLength != NULL) (*scannedLength) += 4;
			(*object)->objectSize += 4;
			if ((strncmp((char *) ptr, "OB", 2) == 0) || (strncmp((char *) ptr, "OW", 2) == 0) || (strncmp((char *) ptr, "SQ", 2) == 0)) {
				/* Tady neni nic */
			} else {
				/* Tady neni nic */
			}
		...
		if (((e.length & 1) != 0) && (e.length != DCM_UNSPECIFIEDLENGTH)) {
			if (debug) (void) DCM_DumpElements(callerObject, 0);
			(void) DCM_CloseObject(callerObject);
			return COND_PushCondition(DCM_UNEVENELEMENTLENGTH, DCM_Message(DCM_UNEVENELEMENTLENGTH), group, element, e.length, "readFile");
		}
		if ((e.length != (U32) DCM_UNSPECIFIEDLENGTH) && (e.length > (U32) size)) {
			if (debug) (void) DCM_DumpElements(callerObject, 0);
			(void) DCM_CloseObject(callerObject);
			return COND_PushCondition(DCM_ELEMENTLENGTHERROR, DCM_Message(DCM_ELEMENTLENGTHERROR), group, element, e.length, size, "readFile");
		}
		if ((e.tag == DCM_DLMITEMDELIMITATIONITEM) || (e.tag == DCM_DLMSEQUENCEDELIMITATIONITEM)) return DCM_NORMAL;

		cond = DCM_LookupElement(&e);
		if (cond != DCM_NORMAL) (void) COND_PopCondition(0);
		...
		if (e.representation == DCM_SQ) {
			...
			if (elementItem->element.d.sq == NULL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "readFile");

			localLength = elementItem->element.length;
			sequenceDone = (localLength == 0);

			while (!sequenceDone) {
				if (debug) fprintf(DEBUG_DEVICE, "Sequence Length: %d %x\n", localLength, localLength);
				if (fileFlag) {
					if (fd != -1) {
				...
				if (size != (long) DCM_UNSPECIFIEDLENGTH) size -= 8;
				fileOffset += (off_t) 8;
				if (scannedLength != NULL) (*scannedLength) += 8;
				(*object)->objectSize += 8;
				if (localLength != DCM_UNSPECIFIEDLENGTH) localLength -= 8;
				...
				ptr += 8;
				if (debug)
					fprintf(DEBUG_DEVICE, "Sequence item: %4x %4x %d (%x)\n", tagGroup, tagElement, tagE.length, tagE.length);
				if (tagE.tag == DCM_DLMITEM) {
/*		    		if (size != DCM_UNSPECIFIEDLENGTH) size -= 8;
					fileOffset += 8;  */

					cond = readFile(name, (fileFlag) ? NULL : ptr, fd, tagE.length, fileOffset, recursionLevel + 1, opt,
									&sequenceObject, &sequenceLength, remainOpenFlag, ctx, rd, sk);
					if (cond == DCM_NORMAL) {
						sequenceItem = CTN_MALLOC(sizeof(*sequenceItem));
						if (sequenceItem == NULL)
							return COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(*sequenceItem), "readFile");
						sequenceItem->object = sequenceObject;

						cond = LST_Enqueue(&elementItem->element.d.sq, sequenceItem);
						if (cond != LST_NORMAL)	return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "readFile");
						if (size != (long) DCM_UNSPECIFIEDLENGTH) size -= sequenceLength;
						fileOffset += (off_t) sequenceLength;
						if (scannedLength != NULL) *scannedLength += sequenceLength;
						(*object)->objectSize += sequenceLength;
						if (localLength != DCM_UNSPECIFIEDLENGTH) localLength -= sequenceLength;
						ptr += sequenceLength;
					}else{
						return cond;
					}
				...
				if (localLength == 0) sequenceDone = TRUE;
			}
		}else{
			...
			if (pixelFlag) {
				if (fileFlag) *remainOpenFlag = TRUE;
				...
				if (fileFlag) {
					if (fd != -1) {
						...
						cond = (*object)->rd((*object)->userCtx, elementItem->element.d.ot, (long) elementItem->element.length, &nBytes);
					}
...
#ifdef LITTLE_ENDIAN_ARCHITECTURE
				if (elementItem->element.representation == DCM_AT) swapATGroupElement(&elementItem->element);
#endif
				if (byteOrder != BYTEORDER_SAME) swapInPlace(object, &elementItem->element);
				if (convertFlag) {
					cond = verifyFormat(elementItem);
					if (cond != DCM_NORMAL)	return cond;
				}
			}
			if (size != (long) DCM_UNSPECIFIEDLENGTH) size -= elementItem->originalDataLength;
			fileOffset += (off_t) elementItem->originalDataLength;
			if (scannedLength != NULL) (*scannedLength) += elementItem->originalDataLength;

			elementItem->paddedDataLength = elementItem->element.length;
			if (elementItem->paddedDataLength & 1) elementItem->paddedDataLength += 1;
		...
		if ((long) DCM_TAG_GROUP(e.tag) == lastGroup) {
			if ((long) DCM_TAG_ELEMENT(e.tag) <= lastElement)
				return COND_PushCondition(DCM_ELEMENTOUTOFORDER, DCM_Message(DCM_ELEMENTOUTOFORDER), group, element, "readFile");
		...
		if (createGroupFlag == TRUE) {
			...
			if (groupItem == NULL) {
				(void) DCM_CloseObject(callerObject);
				return COND_PushCondition(DCM_ELEMENTCREATEFAILED, DCM_Message(DCM_ELEMENTCREATEFAILED), "readFile", group, 0xffff, sizeof(*groupItem));
			}
		...
		if (debug) {
/*lint -e644 */
			(void) fprintf(DEBUG_DEVICE, "Address: %px Group %2x, element %2x, length %ld ", elementItem, DCM_TAG_GROUP(elementItem->element.tag),
					   DCM_TAG_ELEMENT(elementItem->element.tag), elementItem->element.length);
/*lint +e644 */
...
static CONDITION
readPreamble(const char *name, unsigned char **ptr, int fd, U32 * size, off_t * fileOffset, CTNBOOLEAN knownLength, PRIVATE_OBJECT ** object, U32 * scannedLength)
{
    int 		nBytes, tmp;
    CONDITION 	cond;
    char 		label[4];

    if (*size == 0)	return DCM_STREAMCOMPLETE;

    if ((*size < DCM_PREAMBLELENGTH + 4) && knownLength) {
    	if (debug) (void) DCM_DumpElements((DCM_OBJECT **) object, 0);
    ...
    if (*ptr == NULL) {
    	if (fd != -1) {
    		...
    		cond = (*object)->rd((*object)->userCtx, (*object)->preamble, DCM_PREAMBLELENGTH, &nBytes);
    ...
    if (knownLength) *size -= DCM_PREAMBLELENGTH + sizeof(label);
    if (fileOffset != NULL) *fileOffset += (off_t) DCM_PREAMBLELENGTH + sizeof(label);
    if (*ptr != NULL) *ptr += DCM_PREAMBLELENGTH + sizeof(label);

    (*object)->objectSize += DCM_PREAMBLELENGTH + sizeof(label);

    if (strncmp(label, "DICM", 4) != 0) return 0;
...
static CONDITION
readGroupElement(const char *name, unsigned char **ptr, int fd, U32 * size, off_t * fileOffset, CTNBOOLEAN knownLength, int byteOrder,
		 CTNBOOLEAN explicitVR, CTNBOOLEAN acceptVRMismatch, PRIVATE_OBJECT ** object, U32 * scannedLength, DCM_ELEMENT * e)
{
    ...
    if (knownLength) *size -= 4;
    if (fileOffset != NULL) *fileOffset += (off_t) 4;
    if (scannedLength != NULL) (*scannedLength) += 4;
    ...
    if (*ptr != NULL) *ptr += 4;
    if (debug) fprintf(DEBUG_DEVICE, "%04x %04x ", group, element);

    cond = DCM_LookupElement(e);
    if (cond != DCM_NORMAL)	(void) COND_PopCondition(0);

    if (e->representation == DCM_CTX) ctxSensitiveLookup(object, e);

    return DCM_NORMAL;
}

static CONDITION
readVRLength(const char *name, unsigned char **ptr, int fd, U32 * size, off_t * fileOffset, CTNBOOLEAN knownLength, int byteOrder,
			 CTNBOOLEAN explicitVR, CTNBOOLEAN acceptVRMismatch, PRIVATE_OBJECT ** object, U32 * scannedLength, DCM_ELEMENT * e)
{
    unsigned char 	*localPtr;
    unsigned char 	buf[4];
    ...
    if (*ptr == NULL){
    	...
    	if (nBytes != 4) return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), name, "readVRLength");
    	localPtr = buf;
    }else{
    	localPtr = *ptr;
    	*ptr += 4;
    ...
    e->length = 0;
    if (e->representation == DCM_DLM) explicitVR = FALSE;	/* Special rule for delimitors */
    ...
    if (explicitVR) {
    	...
    	if (vrPtr == NULL) return COND_PushCondition(DCM_UNRECOGNIZEDVRCODE, DCM_Message(DCM_UNRECOGNIZEDVRCODE), vrCode, "readVRLength");

    	if (vrPtr->representation != e->representation){
    		if (vrPtr->representation == DCM_UN){
    			/* Kdyz to nevi src system tak uz nikdo :-) Mozna by to chtelo nastavovat podle e */
    			e->representation = vrPtr->representation;
    		}else if (vrPtr->representation == DCM_OB){
    			/* This is probably from the waveform supplement where they transmit as OB and expect us to pull it out later */
    			/* We will just keep our VR which was based on context in the object */
    			e->representation = vrPtr->representation;
    		}else if (e->representation == DCM_UN || e->representation == DCM_CTX || e->representation == DCM_RET || vrPtr->representation == DCM_OW || acceptVRMismatch){
    			/* Believe input */
    			e->representation = vrPtr->representation;
    		}else{
    			if (e->tag != DCM_PXLPIXELDATA) return COND_PushCondition(DCM_VRMISMATCH, DCM_Message(DCM_VRMISMATCH), vrCode, e->tag);
    		}
    	}

    	if (vrPtr->representation != DCM_OW && vrPtr->representation != DCM_OB &&  vrPtr->representation != DCM_UN && vrPtr->representation != DCM_UT && vrPtr->representation != DCM_SQ) {
...
/*	    	if (*ptr != NULL) *ptr += 4; */
    		...
    		if (knownLength) *size -= 4;
    		if (fileOffset != 0) *fileOffset += (off_t) 4;
    		if (scannedLength != NULL) (*scannedLength) += 4;
...
/*		if (*ptr != NULL) *ptr += 4; */
    ...
    if (debug){
    	char localVR[3];
    	(void)mapVRtoASCII(e->representation, localVR);
    	//fprintf(DEBUG_DEVICE, "VR:%s Length:%6.0f FileOffset:%06.0f Description:%s\n", localVR, *fileOffset, e->length, e->description);
    }
    if (((e->length & 1) != 0) && (e->length != DCM_UNSPECIFIEDLENGTH)){
    	if (debug) (void) DCM_DumpElements((DCM_OBJECT **) object, 0);
    	(void) DCM_CloseObject((DCM_OBJECT **) object);
    	return COND_PushCondition(DCM_UNEVENELEMENTLENGTH, DCM_Message(DCM_UNEVENELEMENTLENGTH), DCM_TAG_GROUP(e->tag), DCM_TAG_ELEMENT(e->tag), e->length, "readFile");
    }
    if ((e->length != (U32) DCM_UNSPECIFIEDLENGTH) && (e->length > (U32) (*size))){
    	if (debug) (void) DCM_DumpElements((DCM_OBJECT **) object, 0);
    	(void) DCM_CloseObject((DCM_OBJECT **) object);
    	return COND_PushCondition(DCM_ELEMENTLENGTHERROR, DCM_Message(DCM_ELEMENTLENGTHERROR), DCM_TAG_GROUP(e->tag), DCM_TAG_ELEMENT(e->tag), e->length, *size, "readFile");
    }
    return DCM_NORMAL;
}

static CONDITION
readSequence(const char *name, unsigned char **ptr, int fd, U32 * size, off_t * fileOffset, int recursionLevel, unsigned long opt,
			 int byteOrder, CTNBOOLEAN explicitVR, CTNBOOLEAN acceptVRMismatch, CTNBOOLEAN fileFlag, CTNBOOLEAN * remainOpenFlag,
			 CTNBOOLEAN convertFlag, PRIVATE_OBJECT ** object, U32 * scannedLength, DCM_ELEMENT * e, PRV_ELEMENT_ITEM ** elementItem)
{
    ...
    (*elementItem)->element.d.sq = LST_Create();
    if ((*elementItem)->element.d.sq == NULL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "readSequence");

    localLength = (*elementItem)->element.length;
    sequenceDone = (localLength == 0);

    while (!sequenceDone) {
    	if (debug) fprintf(DEBUG_DEVICE, "Sequence Length: %ld %lx\n", localLength, localLength);
    	sequenceLength = 0;
    	if (fileOffset != NULL) itemTagOffset = *fileOffset;

    	flag = readGroupElement(name, ptr, fd, &localLength, fileOffset, knownLength, byteOrder, explicitVR, acceptVRMismatch, object, &sequenceLength, &tagE);
    	if (flag == DCM_STREAMCOMPLETE){
    		break;
    	}else if (flag != DCM_NORMAL){
    		return flag;
    	}
    	flag = readVRLength(name, ptr, fd, &localLength, fileOffset, knownLength, byteOrder, explicitVR, acceptVRMismatch, object, &sequenceLength, &tagE);
    	if (flag != DCM_NORMAL) return flag;
    	...
    	if (debug) fprintf(DEBUG_DEVICE, "Sequence item: %4x,%4x %ld (%lx)\n", DCM_TAG_GROUP(tagE.tag), DCM_TAG_ELEMENT(tagE.tag), tagE.length, tagE.length);
    	if (tagE.tag == DCM_DLMITEM) {
    		localPtr = *ptr;
    		cond = readFile1(name, localPtr, fd, tagE.length, fileOffset, recursionLevel + 1, opt,
							 object, &sequenceObject, &sequenceLength, remainOpenFlag, (*object)->userCtx, (*object)->rd, (*object)->sk);
    		if (!fileFlag) *ptr = localPtr + sequenceLength;
    		if (cond == DCM_NORMAL) {
    			sequenceItem = CTN_MALLOC(sizeof(*sequenceItem));
    			if (sequenceItem == NULL)
    				return COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(*sequenceItem), "readFile");
...
static CONDITION
scanCompressedPixels(char *name, unsigned char **ptr, int fd, U32 * size, off_t * fileOffset, int recursionLevel, unsigned long opt,
		     int byteOrder, CTNBOOLEAN explicitVR, CTNBOOLEAN acceptVRMismatch, CTNBOOLEAN fileFlag, CTNBOOLEAN * remainOpenFlag,
		     CTNBOOLEAN convertFlag, PRIVATE_OBJECT ** object, U32 * scannedLength, DCM_ELEMENT * e, PRV_ELEMENT_ITEM ** elementItem)
{
    CTNBOOLEAN 		knownLength = TRUE;
    ...
    while (!sequenceDone) {
    	sequenceLength = 0;
    	flag = readGroupElement(name, ptr, fd, &localLength, fileOffset, FALSE, byteOrder, explicitVR, acceptVRMismatch, object, &sequenceLength, &tagE);
    	if (flag == DCM_STREAMCOMPLETE){
    		break;
    	}else if (flag != DCM_NORMAL){
    		return flag;
    	...
    	if (tagE.tag == DCM_DLMITEM) {
    		...
    		if (tagE.length != 0) {
    			...
    			*fileOffset += tagE.length;
    			...
    			if (*size != (long) DCM_UNSPECIFIEDLENGTH) *size -= tagE.length;
    			if (scannedLength != NULL) *scannedLength += tagE.length;
    		}
    	}else{
    		sequenceDone = TRUE;
    	}
    	if (localLength == 0) sequenceDone = TRUE;
    	if (debug) fprintf(DEBUG_DEVICE, "Scanned Bytes: %ld\n", scannedBytes);
    }
    if ((scannedBytes & 1) != 0) {
    	lseek(fd, 1, SEEK_CUR);
    	*fileOffset += 1;
    	if (*size != (long) DCM_UNSPECIFIEDLENGTH) *size -= 1;
    }
    return DCM_NORMAL;
}

static CONDITION
readData(const char *name, unsigned char **ptr, int fd, U32 * size, off_t * fileOffset, CTNBOOLEAN knownLength, int byteOrder,
		 CTNBOOLEAN explicitVR, CTNBOOLEAN acceptVRMismatch, CTNBOOLEAN fileFlag, CTNBOOLEAN * remainOpenFlag,
		 CTNBOOLEAN convertFlag, PRIVATE_OBJECT ** object, U32 * scannedLength, DCM_ELEMENT * e, PRV_ELEMENT_ITEM ** elementItem)
{
...
/*		if (fileFlag) */
    ...
    if (*size != (long) DCM_UNSPECIFIEDLENGTH) *size -= (*elementItem)->originalDataLength;
    if (fileOffset != NULL) *fileOffset += (off_t) (*elementItem)->originalDataLength;
    if (scannedLength != NULL) (*scannedLength) += (*elementItem)->originalDataLength;
    if ((*elementItem)->element.length != DCM_UNSPECIFIEDLENGTH)
    	(*elementItem)->paddedDataLength = (*elementItem)->element.length;
...
static CONDITION
checkAttributeOrder(DCM_ELEMENT * e, long *lastGroup, long *lastElement, CTNBOOLEAN allowRepeatElements)
{
    ...
    if ((long) group == *lastGroup) {
    	if (((long) element == *lastElement) && allowRepeatElements) return DCM_REPEATEDELEMENT;
    	if ((long) element <= *lastElement) return COND_PushCondition(DCM_ELEMENTOUTOFORDER, DCM_Message(DCM_ELEMENTOUTOFORDER), group, element, "checkAttributeOrder");
    }else if ((long) group > *lastGroup){
    	/* NIC */
    }else{
    	return COND_PushCondition(DCM_ELEMENTOUTOFORDER, DCM_Message(DCM_ELEMENTOUTOFORDER), group, element, "checkAttributeOrder");
    }
    *lastGroup = (long) group;
    *lastElement = (long) element;
...
static CONDITION
handleGroupItem(PRIVATE_OBJECT ** obj, PRV_GROUP_ITEM ** groupItem, unsigned short group)
{
    ...
    if (createGroupFlag == TRUE) {
    	*groupItem = CTN_MALLOC(sizeof(**groupItem));
    	if (*groupItem == NULL) {
    		...
    		return COND_PushCondition(DCM_ELEMENTCREATEFAILED, DCM_Message(DCM_ELEMENTCREATEFAILED), "handleGroupItem",
...
static CONDITION
readFile1(const char *name, unsigned char *callerBuf, int fd, U32 size, off_t * fileOffset, int recursionLevel,
		  unsigned long opt, PRIVATE_OBJECT ** parentObject, DCM_OBJECT ** callerObject, U32 * scannedLength, CTNBOOLEAN * remainOpenFlag, void *ctx,
		  CONDITION(*rd) (void *ctx, void *buf, int toRead, int *bytesRead), CONDITION(*sk) (void *ctx, int offset, int flag))
{
    CONDITION	    	cond;
    int		        	byteOrder;
    long	        	lastGroup = -1, lastElement = -1;
    U32					sequenceLength,	scannedSequenceLength;
    PRIVATE_OBJECT		** object;
    PRV_GROUP_ITEM		* groupItem = NULL;
    DCM_ELEMENT			e;
    CTNBOOLEAN			convertFlag = FALSE, done = FALSE, knownLength = TRUE, explicitVR = FALSE, acceptVRMismatch = FALSE, part10Flag = FALSE;
    ...
    if ((opt & DCM_CONVERTMASK) == DCM_FORMATCONVERSION) convertFlag = TRUE;
    if ((opt & DCM_VRMASK) == DCM_ACCEPTVRMISMATCH)	acceptVRMismatch = TRUE;
    ...
    if (size == (long) DCM_UNSPECIFIEDLENGTH) knownLength = FALSE;
    if ((fileFlag) && ((opt & DCM_DELETEMASK) == DCM_DELETEONCLOSE) && (recursionLevel == 0)) (*object)->deleteFlag = TRUE;
    if (parentObject != NULL) (*object)->pixelRepresentation = (*parentObject)->pixelRepresentation;

    if (recursionLevel == 0 && part10Flag) {
    	flag = readPreamble(name, &ptr, fd, &size, fileOffset, knownLength, object, scannedLength);
    	if (flag != DCM_NORMAL) goto abort;
    }

    while (!done){
    	flag = readGroupElement(name, &ptr, fd, &size, fileOffset, knownLength, byteOrder, explicitVR, acceptVRMismatch, object, scannedLength, &e);
    	if (flag == DCM_STREAMCOMPLETE){
    		break;
    	}else if (flag != DCM_NORMAL){
    		goto abort;
...
#if 0
    	if (e.tag == DCM_MAKETAG(0x7fe0, 0x0010)) fprintf(stderr, "Found pixels\n");
#endif
    	flag = readVRLength(name, &ptr, fd, &size, fileOffset, knownLength,	byteOrder, explicitVR, acceptVRMismatch, object, scannedLength, &e);
    	if (flag != DCM_NORMAL) goto abort;

    	if ((e.representation == DCM_UN) && (e.length == DCM_UNSPECIFIEDLENGTH)) e.representation = DCM_SQ;
    	...
    	if (e.tag == DCM_DLMSEQUENCEDELIMITATIONITEM) return DCM_NORMAL;
#endif

    	if (e.representation == DCM_SQ) {
    		sequenceLength = e.length;
    		scannedSequenceLength = 0;
    		flag = readSequence(name, &ptr, fd, &sequenceLength, fileOffset, recursionLevel, opt,
								byteOrder, explicitVR, acceptVRMismatch, fileFlag, remainOpenFlag,
								convertFlag, object, &scannedSequenceLength, &e, &elementItem);
    		if (flag != DCM_NORMAL)	goto abort;

    		if (size != (long) DCM_UNSPECIFIEDLENGTH) size -= scannedSequenceLength;
    		if (scannedLength != NULL) *scannedLength += scannedSequenceLength;

    	}else{
    		flag = readData(name, &ptr, fd, &size, fileOffset, knownLength, byteOrder, explicitVR, acceptVRMismatch, fileFlag,
							remainOpenFlag, convertFlag, object, scannedLength, &e, &elementItem);
    		if (flag != DCM_NORMAL)	goto abort;
    	}
    	computeVM(object, &elementItem->element);
...
locateElement(PRIVATE_OBJECT ** obj, DCM_TAG tag)
{
    ...
    elementItem = LST_Head(&groupItem->elementList);
    if (elementItem == NULL) return NULL;
...
**
** Purpose:
**	Lookup representation of elements that are context sensitive DCM_CTX
**
** Parameter Dictionary:
...
ctxSensitiveLookup(PRIVATE_OBJECT ** object, DCM_ELEMENT * element)
{
    switch (element->tag) {
		...
		case DCM_IMGLARGESTIMAGEPIXELVALUE:
													element->representation = DCM_CTX;
													break;
...
static CONDITION
copyData(PRIVATE_OBJECT ** object, PRV_ELEMENT_ITEM * from, DCM_ELEMENT * to, U32 * rtnLength)
{
    ...
    if (from->element.representation == DCM_SQ)
    	return COND_PushCondition(DCM_CANNOTGETSEQUENCEVALUE, DCM_Message(DCM_CANNOTGETSEQUENCEVALUE),
								  from->element.tag, "copyData (DCM internal)");

    l = MIN(from->element.length, to->length);
    if (rtnLength != NULL) *rtnLength = l;

    if (from->element.d.ot == NULL) {
    	if ((*object)->fd != -1) {
    		(void) lseek((*object)->fd, from->dataOffset + (off_t) p, SEEK_SET);
    		nBytes = read((*object)->fd, to->d.ot, (int) l);
    	}else{
    		(*object)->sk((*object)->userCtx, (long) (from->dataOffset + (off_t) p), SEEK_SET);
    		cond = (*object)->rd((*object)->userCtx, to->d.ot, (long) l, &nBytes);
    	}
    	if (nBytes != (int) l) {
    		return COND_PushCondition(DCM_FILEACCESSERROR, DCM_Message(DCM_FILEACCESSERROR), (*object)->fileName, "copyData (DCM internal)");
    	}
#ifdef LITTLE_ENDIAN_ARCHITECTURE
    	...
    	unsigned char 	*q;
    	q = (unsigned char *) from->element.d.ot + (U32) p;
...
static void
compareGroup(PRV_GROUP_ITEM * g1, PRV_GROUP_ITEM * g2, void (*callback) (const DCM_ELEMENT * e1,
			 const DCM_ELEMENT * e2, void *ctx), void *ctx)
{
    PRV_ELEMENT_ITEM 	*e1 = NULL, *e2 = NULL;

    if (g1 != NULL) {
    	e1 = LST_Head(&g1->elementList);
    	if (e1 != NULL) LST_Position(&g1->elementList, e1);
    }
    if (g2 != NULL) {
    	e2 = LST_Head(&g2->elementList);
    	if (e2 != NULL) LST_Position(&g2->elementList, e2);
    }
...
CONDITION
DCM_GetCompressedValue(DCM_OBJECT ** callerObject, DCM_TAG tag, void *buf, size_t bufSize, DCM_GET_COMPRESSED_CALLBACK* callback, void *ctx)
{
    PRIVATE_OBJECT		** object;
    PRV_ELEMENT_ITEM	* elementItem;
    S32 				nBytes;
    ...
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag),
								  DCM_TAG_ELEMENT(tag), "DCM_GetEncodedValue");
    ...
    while (elementLength != 0) {
    	sequenceLength = 0;
    	memset(&e, 0, sizeof(e));
    	flag = readGroupElement("", &ptr, (*object)->fd, &size, &fileOffset, FALSE, byteOrder, explicitVR, acceptVRMismatch,
    	...
    	flag = readVRLength("", &ptr, (*object)->fd, &size, &fileOffset, FALSE,	/* Known length */
    	...
    	if (firstBuffer) {
    		...
    		if (e.length != 0) {
    			...
    			if (offsetBuffer == NULL)  exit(1);	/* repair */
    			nBytes = read((*object)->fd, offsetBuffer, e.length);
    			if (nBytes != e.length) exit(1);	/* repair */
    		...
    		for (j = 0; j < offsetBufferCount; j++) {
    			if (streamOffset == offsetBuffer[j]) index = j + 1;
    		}
    		startOfFragment = 1;
    		...
    		while (l != 0) {
    			...
    			if (nBytes != toRead) exit(1);	/* repair */
...
DCM_PrintSequenceList(DCM_OBJECT ** object, DCM_TAG tag)
{
    ...
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag),
								  DCM_TAG_ELEMENT(tag), "DCM_PrintSequenceList");
...
DCM_GetSequenceByOffset(DCM_OBJECT ** object, DCM_TAG tag, unsigned long offset, DCM_OBJECT ** rtnObject)
{
    ...
    if (elementItem == NULL)
    	return COND_PushCondition(DCM_ELEMENTNOTFOUND, DCM_Message(DCM_ELEMENTNOTFOUND), DCM_TAG_GROUP(tag), DCM_TAG_ELEMENT(tag), "DCM_PrintSequenceList");
...
DCM_CopyObject(DCM_OBJECT ** src, DCM_OBJECT ** dst)
{
    ...
    if (dstObj == NULL) {
    	(void) COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(PRIVATE_OBJECT), "DCM_CopyObject");
    	*dst = NULL;
    	return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_CopyObject");
    }
    ...
    if (groupItem != NULL) (void) LST_Position(&(*srcObj)->groupList, groupItem);
...
DCM_MergeObject(DCM_OBJECT ** src, DCM_OBJECT ** dst)
{
    ...
    if (dstObj == NULL) {
    	(void) COND_PushCondition(DCM_MALLOCFAILURE, DCM_Message(DCM_MALLOCFAILURE), sizeof(PRIVATE_OBJECT), "DCM_MergeObject");
    	*dst = NULL;
    	return COND_PushCondition(DCM_OBJECTCREATEFAILED, DCM_Message(DCM_OBJECTCREATEFAILED), "DCM_MergeObject");
    }
    srcObj = (PRIVATE_OBJECT **) src;
...
DCM_GetFirstElement(DCM_OBJECT ** callerObject, DCM_ELEMENT** e)
{
  ...
  if (elementItem == NULL) return DCM_GetNextElement(callerObject, e);
...
DCM_GetNextElement(DCM_OBJECT ** callerObject, DCM_ELEMENT** e)
{
  ...
  if (elementItem == 0) return DCM_GetNextElement(callerObject, e);
...
DCM_AddFragment(DCM_OBJECT** callerObject, void* fragment, U32 fragmentLength)
{
  ...
  if ((fragmentLength & 1) != 0)
	  return COND_PushCondition(DCM_UNEVENFRAGMENTLENGTH, DCM_Message(DCM_UNEVENFRAGMENTLENGTH), fragmentLength, "DCM_AddFragment");
  ...
  if (cond != DCM_NORMAL) return COND_PushCondition(DCM_INSERTFAILED, DCM_Message(DCM_INSERTFAILED), 0x7fe0, 0x0010, "DCM_AddFragment");
  ...
  if (elementItem == NULL) {
	  ...
	  if (newItem->element.d.fragments == NULL) return COND_PushCondition(DCM_LISTFAILURE, DCM_Message(DCM_LISTFAILURE), "DCM_AddFragment");
