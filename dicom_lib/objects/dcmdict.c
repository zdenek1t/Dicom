...
/*    {DCM_IDLENGTHTOEND, DCM_RET, "ID Length to End (RET)"}, */
    ...
    {DCM_IDPRIMARYANATOMICSTRUCTURESEQ, DCM_SQ, "ID Primary Anatomic Structure Sequence"},
    {DCM_IDPRIMARYANATOMICSTRUCTUREMODIFIERSEQ, DCM_SQ, "ID Primary Anatomic Structure Modifier Sequence"},
...
static DCMDICT ACQ_dictionary[] = {
    ...
    {DCM_ACQSECONDARYCAPTMANUFACTURER, DCM_LO, "ACQ Secondary Capture Device Manufacturer"},
    {DCM_ACQSECONDARYCAPTMODEL, DCM_LO, "ACQ Secondary Capture Device Model Name"},
    {DCM_ACQSECONDARYCAPTSOFTWAREVERSION, DCM_LO, "ACQ Secondary Capture Device Software Version"},
    ...
    {DCM_ACQRADIOPHARMACEUTICALSPECIFICACTIVITY, DCM_DS, "ACQ Radiopharmaceutical Specific Activity"},
    ...
    {DCM_ACQACQUISITIONTIMESYNCHRONIZED, DCM_CS, "ACQ Acquisition Time Synchronized"},	/* Sup 30 0.7 */
    {DCM_ACQTIMESOURCE, DCM_SH, "ACQ Time Source"},	/* Sup 30 0.7 */
    {DCM_ACQTIMEDISTRIBUTIONPROTOCOL, DCM_CS, "ACQ Time Distribution Protocol"},	/* Sup 30 0.7 */
    ...
    {DCM_ACQSOFTTISSUEFOCUSTHERMALINDEX, DCM_DS, "ACQ Soft Tissue-focus Thermal Index"},
    {DCM_ACQSOFTTISSUESURFACETHERMALINDEX, DCM_CS, "ACQ Soft Tissue-surface Thermal Index"},
    ...
    {DCM_ACQIMAGETRANSFORMATIONMATRIX, DCM_DS, "ACQ Image Transformation Matrix"},
...
static DCMDICT REL_dictionary[] = {
    ...
    {DCM_RELNUMBERPATRELATEDSTUDIES, DCM_IS, "REL Number of Patient Related Studies"},
    ...
    {DCM_RELMODIFIEDIMAGEDESCRIPTION, DCM_RET, "REL Modified Image Description (RET)"},
    {DCM_RELIMAGECOMMENTS, DCM_LT, "REL Image Comments"},
    {DCM_RELORIGINALIMAGEID, DCM_RET, "REL Original Image ID (RET)"},
    {DCM_RELORIGINALIMAGEIDNOMENCLATURE, DCM_RET, "REL Orig Image ID Nomenclature (RET)"}
};
...
static DCMDICT SDY_dictionary[] = {
    ...
    {DCM_SDYSCHEDULEDLOCATIONAETITLE, DCM_AE, "SDY Scheduled Study Location AE Title(s)"},
...
static DCMDICT NMI_dictionary[] = {
    ...
    {DCM_NMIRANDOMSCORRECTIONMETHOD, DCM_CS, "NMI Randoms Correction Method"},	/* 1100 */
    {DCM_NMIATTENUATIONCORRECTIONMETHOD, DCM_LO, "NMI Attenuation Correction Method"},	/* 1101 */
    {DCM_NMIDECAYCORRECTION, DCM_CS, "NMI Decay Correction"},	/* 1102 */
    {DCM_NMIRECONSTRUCTIONMETHOD, DCM_LO, "NMI Reconstruction Method"},	/* 1103 */
    {DCM_NMIDETECTORLINESRESPONSEUSED, DCM_LO, "NMI Detector Lines of Response Used"},	/* 1104 */
    ...
    {DCM_NMIPRIMARYCOUNTSACCUMULATED, DCM_IS, "NMI Primary (Prompts) Counts Accumulated"},	/* 1310 */
    {DCM_NMISECONDARYCOUNTSACCUMULATED, DCM_IS, "NMI Secondary Counts Accumulated"},	/* 1311 */
    ...
    {DCM_NMIDEADTIMECORRECTIONFLAG, DCM_CS, "NMI Dead Time Correction Flag"},	/* 1401 */
};
...
/* Define the entries in the 0x3004 group, Dose Volume Histogram (DVH), used in RT planning. */
...
/* Define the entries in the 0x3006 group, Structure Set, used in RT planning. */
...
/* Define the outer layer dictionary which contains group numbers and pointers to each of the individual group lists. */
static GROUPPTR group_dictionary[] = {
    ...
    {DCM_GROUPIDENTIFYING, sizeof(ID_dictionary) / sizeof(DCMDICT), ID_dictionary},
    {DCM_GROUPPATIENTINFO, sizeof(PAT_dictionary) / sizeof(DCMDICT), PAT_dictionary},
    {DCM_GROUPACQUISITION, sizeof(ACQ_dictionary) / sizeof(DCMDICT), ACQ_dictionary},
    {DCM_GROUPRELATIONSHIP, sizeof(REL_dictionary) / sizeof(DCMDICT), REL_dictionary},
    ...
    {DCM_GROUPWAVEFORM, sizeof(WAV_dictionary) / sizeof(DCMDICT), WAV_dictionary},
    {DCM_GRPPROCEDURE, sizeof(PRC_dictionary) / sizeof(DCMDICT), PRC_dictionary},
    {DCM_GROUPDEVICE, sizeof(DEV_dictionary) / sizeof(DCMDICT), DEV_dictionary},
    {DCM_GROUPNMIMAGE, sizeof(NMI_dictionary) / sizeof(DCMDICT), NMI_dictionary},
    {DCM_GROUPGRAPHICS, sizeof(GRP_dictionary) / sizeof(DCMDICT), GRP_dictionary},
    {DCM_GROUPMEDIA, sizeof(MED_dictionary) / sizeof(DCMDICT), MED_dictionary},
    {DCM_GROUPBASICFILMSESSION, sizeof(BFS_dictionary) / sizeof(DCMDICT), BFS_dictionary},
    {DCM_GROUPBASICFILMBOX, sizeof(BFB_dictionary) / sizeof(DCMDICT), BFB_dictionary},
    {DCM_GROUPBASICIMAGEBOX, sizeof(BIB_dictionary) / sizeof(DCMDICT), BIB_dictionary},
    {DCM_GROUPBASICANNOTATIONBOX, sizeof(BAB_dictionary) / sizeof(DCMDICT), BAB_dictionary},

    {DCM_GROUPBASICIMAGEOVERLAYBOX, sizeof(IOB_dictionary) / sizeof(DCMDICT), IOB_dictionary},
    ...
    {DCM_GROUPPRINTER, sizeof(PRN_dictionary) / sizeof(DCMDICT), PRN_dictionary},
...
/*  Add this entry in when we define retired attributes in text group. */
#if 0
    {DCM_GROUPTEXT, sizeof(TXT_dictionary) / sizeof(DCMDICT), TXT_dictionary},
#endif
    {DCM_GROUPRESULTS, sizeof(RES_dictionary) / sizeof(DCMDICT), RES_dictionary},
    {DCM_GROUPCURVE, sizeof(CRV_dictionary) / sizeof(DCMDICT), CRV_dictionary},
    {DCM_GROUPOVERLAY, sizeof(OLY_dictionary) / sizeof(DCMDICT), OLY_dictionary},
    ...
    {DCM_GROUPDELIMITER, sizeof(DLM_dictionary) / sizeof(DCMDICT), DLM_dictionary}
};
...
DCM_LookupElement(DCM_ELEMENT * element)
{
    ...
    for (index = 0, p = NULL; index < sizeof(group_dictionary) / sizeof(group_dictionary[0]) && p == NULL; index++){
    	if (DCM_TAG_GROUP(element->tag) == group_dictionary[index].group) p = &group_dictionary[index];
    }
    if (p == NULL) {
    	...
    	return COND_PushCondition(DCM_UNRECOGNIZEDGROUP, DCM_Message(DCM_UNRECOGNIZEDGROUP), DCM_TAG_GROUP(element->tag), "DCM_LookupElement");
    }
    entries = p->entries;
    ...
    if (!found)	return COND_PushCondition(DCM_UNRECOGNIZEDELEMENT, DCM_Message(DCM_UNRECOGNIZEDELEMENT), DCM_TAG_GROUP(element->tag), DCM_TAG_ELEMENT(element->tag), "DCM_LookupElement");
...
CONDITION
DCM_GroupDictionary(unsigned short group, void *ctx, void (*callback) (unsigned short g, char *description, void *ctx))
{
    int 	i;

    for (i = 0; i < (int) DIM_OF(groupTable); i++) {
    	if ((group == 0xffff) || (group == groupTable[i].group)) callback(groupTable[i].group, groupTable[i].description, ctx);
    }
    return DCM_NORMAL;
}
...
CONDITION
DCM_ElementDictionary(DCM_TAG tag, void *ctx, void (*callback) (DCM_TAG t, char *description, DCM_VALUEREPRESENTATION r, void *ctx))
{
    int 			i;
    unsigned long 	j;
    GROUPPTR 		*p;
    ...
    for (i = 0; i < (int) DIM_OF(group_dictionary); i++) {
    	if ((DCM_TAG_GROUP(tag) == group_dictionary[i].group) || (DCM_TAG_GROUP(tag) == 0xffff)) {
    		p = &group_dictionary[i];
    		dictionaryPtr = p->dict;
    		for (j = 0; j < p->entries; j++, dictionaryPtr++) {
    			if ((DCM_TAG_ELEMENT(tag) == 0xffff) || (DCM_TAG_ELEMENT(tag) == DCM_TAG_ELEMENT(dictionaryPtr->tag))) {
    				callback(dictionaryPtr->tag, dictionaryPtr->englishDescription, dictionaryPtr->representation, ctx);
    			}
    		}
    	}
    }
