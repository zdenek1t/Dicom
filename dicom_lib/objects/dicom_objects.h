...
#define DCM_MAKETAG(g, e) (((((unsigned int)(g)) << 16) & 0xffff0000) | ((unsigned int)(e) & 0xffff))
#else
typedef unsigned long DCM_TAG;
#define DCM_MAKETAG(g, e) (((((unsigned long)(g)) << 16) & 0xffff0000) | ((unsigned long)(e) & 0xffff))
#endif
...
*/
CONDITION
DCM_OpenFile(const char *name, unsigned long opt, DCM_OBJECT ** object);
CONDITION
DCM_CreateObject(DCM_OBJECT ** obj, unsigned long opt);
CONDITION
DCM_CopyObject(DCM_OBJECT ** src, DCM_OBJECT** dst);
CONDITION
DCM_MergeObject(DCM_OBJECT ** src, DCM_OBJECT** dst);
CONDITION
DCM_AddElement(DCM_OBJECT ** obj, DCM_ELEMENT * ele);
CONDITION
DCM_AddSequenceElement(DCM_OBJECT ** obj, DCM_ELEMENT * ele);
CONDITION
DCM_RemoveElement(DCM_OBJECT ** obj, DCM_TAG tag);
CONDITION
DCM_RemoveGroup(DCM_OBJECT ** callerObject, unsigned short group);
CONDITION
DCM_ImportStream(unsigned char *buf, unsigned long length, unsigned long opt, DCM_OBJECT ** rntObj);
CONDITION
DCM_ReadStream(DCM_OBJECT ** obj, unsigned long opt, long size, void *ctx, CONDITION(*rd) (void *ctx, void *buf, int toRead, int *bytesRead), CONDITION(*sk) (void *ctx, int offset, int flag));
CONDITION
DCM_CloseObject(DCM_OBJECT ** obj);

typedef CONDITION(DCM_EXPORT_STREAM_CALLBACK) (void *buf, U32 bytesExported, int lastFlag, void *ctx);

CONDITION
DCM_ExportStream(DCM_OBJECT ** obj, unsigned long opt, void *buf, unsigned long maxlen, DCM_EXPORT_STREAM_CALLBACK* callback, void *ctx);
CONDITION
DCM_GetElementValue(DCM_OBJECT ** obj, DCM_ELEMENT * element, U32 * rtnLength, void **ctx);
char*
...
CONDITION
DCM_GetElementValueOffset(DCM_OBJECT **obj, DCM_ELEMENT *element, unsigned long offset);
...
CONDITION
DCM_GetCompressedValue(DCM_OBJECT ** obj, DCM_TAG tag, void *buf, size_t bufSize, DCM_GET_COMPRESSED_CALLBACK *callback, void *ctx);

CONDITION
DCM_GetObjectSize(DCM_OBJECT ** obj, unsigned long *retlen);
CONDITION
DCM_DumpElements(DCM_OBJECT ** obj, long vm);
CONDITION
DCM_FormatElements(DCM_OBJECT ** obj, long vm, const char* prefix);
CONDITION
DCM_LookupElement(DCM_ELEMENT * attribute);
CONDITION
...
char
*DCM_Message(CONDITION cond);
void
DCM_Debug(CTNBOOLEAN flag);
CONDITION
DCM_WriteFile(DCM_OBJECT ** obj, unsigned long opt, const char *file);
CONDITION 
DCM_ComputeExportLength(DCM_OBJECT ** obj, unsigned long opt, unsigned long *length);
CONDITION
DCM_ModifyElements(DCM_OBJECT ** obj, DCM_ELEMENT * element, int count, DCM_FLAGGED_ELEMENT * flaggedElement, int flagCount, int *updateCount);
CONDITION
DCM_ListToString(LST_HEAD * list, long offset, char **string);
CONDITION
DCM_ParseObject(DCM_OBJECT ** obj, DCM_ELEMENT * list, int count, DCM_FLAGGED_ELEMENT * flaggedElement, int flagCount, int *parseCount);
CONDITION
DCM_ScanParseObject(DCM_OBJECT ** object, void *buf, size_t bufferSize, DCM_FLAGGED_ELEMENT * elementVector, int vectorLength, CONDITION(*callback) (const DCM_ELEMENT* e, void* ctx), void *ctx);
CTNBOOLEAN
...
#define DCM_NORMAL						FORM_COND(FAC_DCM, SEV_SUCC, 1)	/* Normal return from DCM package */
#define DCM_FILEOPENFAILED				FORM_COND(FAC_DCM, SEV_ERROR, 2)/* Failed to open file requested by caller */
#define DCM_FILEACCESSERROR				FORM_COND(FAC_DCM, SEV_ERROR, 3)/* Error accessing open file */
#define DCM_OBJECTCREATEFAILED			FORM_COND(FAC_DCM, SEV_ERROR, 4)/* Failed to create a new DCM object */
#define DCM_NULLOBJECT					FORM_COND(FAC_DCM, SEV_ERROR, 5)/* Null object passed to DCM routine */
#define DCM_ILLEGALOBJECT				FORM_COND(FAC_DCM, SEV_ERROR, 6)/* Illegal object passed to DCM routine */
#define DCM_ELEMENTNOTFOUND				FORM_COND(FAC_DCM, SEV_ERROR, 7)/* Requested element not found in object */
#define DCM_ILLEGALSTREAMLENGTH			FORM_COND(FAC_DCM, SEV_ERROR, 8)/* Illegal length for stream (too short) */
#define DCM_ELEMENTCREATEFAILED			FORM_COND(FAC_DCM, SEV_ERROR, 9)/* Failed to create a new DCM element */
#define DCM_UNRECOGNIZEDGROUP			FORM_COND(FAC_DCM, SEV_WARN, 10)/* Unrecognized group */
#define DCM_UNRECOGNIZEDELEMENT			FORM_COND(FAC_DCM, SEV_WARN, 11)/* Unrecognized element */
#define DCM_ELEMENTOUTOFORDER			FORM_COND(FAC_DCM, SEV_ERROR, 12)/* Element out of order in DCM stream */
#define DCM_LISTFAILURE					FORM_COND(FAC_DCM, SEV_ERROR, 13)/* Failure by a list routine */
#define	DCM_ILLEGALOPTION				FORM_COND(FAC_DCM, SEV_ERROR, 14)/* Illegal option for processing stream */
#define	DCM_ILLEGALADD					FORM_COND(FAC_DCM, SEV_ERROR, 19)/* Illegal Add of an element */
#define	DCM_GETINCOMPLETE				FORM_COND(FAC_DCM, SEV_WARN, 20)/* Incomplete data get operation */
#define	DCM_ILLEGALCONTEXT				FORM_COND(FAC_DCM, SEV_ERROR, 21)/* Illegal context value */
#define	DCM_ILLEGALREPRESENTATION		FORM_COND(FAC_DCM, SEV_ERROR, 22)/* Illegal rep given by caller */
#define	DCM_UNEVENELEMENTLENGTH			FORM_COND(FAC_DCM, SEV_ERROR, 23)/* Uneven element data length */
#define	DCM_ELEMENTLENGTHERROR			FORM_COND(FAC_DCM, SEV_ERROR, 25)/* Element length > remaining bytes in data */
#define	DCM_GROUPNOTFOUND				FORM_COND(FAC_DCM, SEV_ERROR, 27)/* Did not find requested group */
#define	DCM_FILECREATEFAILED			FORM_COND(FAC_DCM, SEV_ERROR, 28)/* Failed to create a file */
#define	DCM_FILEIOERROR					FORM_COND(FAC_DCM, SEV_ERROR, 29)/* File I/O error */
#define	DCM_INSERTFAILED				FORM_COND(FAC_DCM, SEV_ERROR, 30)/* Failed to insert a new element */
...
/* Define elements for the Procedure Step group (0040), General Purpose Scheduled/Performed Procedure Step Info */
