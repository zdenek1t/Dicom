    ...
    while (regex[i] != '\000') {
    	switch (regex[i]) {
			...
			case '?':		/* Transform the "?" to "." or "?" if escaped */
						...
						i++;
						break;
			case '\\':		/* Note that we have seen the escape character */
						switch (escape_on) {
							case OFF:
										escape_on = ON;
										break;
						...
						break;
			case '.':
			case '[':		/* These are special to regex and need to be escaped */
			case ']':
						new_regex[j++] = '\\';
						new_regex[j++] = regex[i++];
						escape_on = OFF;
...
#endif
/* UTL_ConvertDatetoLong Convert a Dicom date to a long for comparision ease. */
long
...
/* UTL_ConvertLongtoDate Convert a long to a Dicom date. */
void
...
/* UTL_ConvertTimetoFloat Convert a Dicom time to a floating point number for comparision ease. */
double
UTL_ConvertTimetoFloat(const char *time)
{

    size_t	    	i;
    char	    	hour[3], minute[3], second[3], fracsec[7];
    const char 		*p;
    double	        divisor, hh, mm, ss, fs;
...
/* UTL_ConvertFloattoTime Convert a floating point number to a Dicom time. */
void
UTL_ConvertFloattoTime(double dt, char *time)
{
    int        hour, minute, second, fracsec;
...
/* UTL_DateMatch Match a date range as specified in the Dicom standard */
CONDITION
UTL_DateMatch(char *datestring, char *stm)
{
    ...
    int	        match;
    char	    *ndate;
    long	    start_date, end_date, date_in_question;

    if ((ndate = (char *) malloc(strlen(datestring) + 1)) == (char *) NULL)	return (UTL_NOMATCH);
...
UTL_TimeMatch(char *timestring, char *stm)
{
    ...
    int        match;
    char       *ntime;
    double     start_time, end_time, time_in_question;

    if ((ntime = (char *) malloc(strlen(timestring) + 2)) == (char *) NULL)	return (UTL_NOMATCH);
...
UTL_DeltaTime(void *timeStamp)
{
    ...
    if (strcmp(t->key, "UTL STAMP") != 0) return -1.0;
...
UTL_ReleaseTimeStamp(void *timeStamp)
{
    ...
    if (strcmp(t->key, "UTL STAMP") != 0) return;
...
CONDITION UTL_ScanDirectory(const char* path, LST_HEAD** lst)
{
  ...
  *lst = LST_Create();
  ...
  if( (hFile = _findfirst(directoryText, &fileInfo)) == -1L) return 0;
...
CONDITION UTL_ReadConfigFile( )
{
  ...
  if (UTL_configList != 0) return UTL_NORMAL;

  UTL_configList = LST_Create();
  if (UTL_configList == NULL) return 0;

  if (UTL_configFile == 0) return UTL_NORMAL;

  if (UTL_configFile[0] == '\0') return UTL_NORMAL;
  ...
  while (fgets(buf, sizeof(buf), f) != NULL) {
	  ...
	  item = (CONFIG_ITEM*)malloc(sizeof(*item) + strlen(token1) + strlen(token2) + 2);
...
{
  if (UTL_configFile != 0) CTN_FREE(UTL_configFile);

  if (configFile == 0 || configFile[0] == '\0') {
	  char* p = getenv("DCM_CFG");
	  if (p == NULL) return UTL_NO_CTN_TARGET;

	  UTL_configFile = (char*) malloc(strlen(p) + strlen("/etc/dcm_srv.cfg") + 1);
	  strcpy(UTL_configFile, p);
	  strcat(UTL_configFile, "/etc/dcm_srv.cfg");
  }else{
...
char*
UTL_GetConfigParameter(const char* paramName)
{
  CONDITION 		cond;
  char 				nameCopy[256];
  CONFIG_ITEM* 		item;
  ...
  if (cond != UTL_NORMAL) return NULL;
  ...
  while(item != NULL) {
	  if (strcmp(item->pName, paramName) == 0) return item->pValue;
...
char**
UTL_ExpandToPointerArray(const char* inputText, const char* delimiters, int* numberOfEntries)
{
  int 		idx;
  int 		memorySize = 0;
  int 		arrayIndex = 0;
...
UTL_IsFile(const char* path)
{
  ...
  i = _stat(path, &buf);
  if (i == 0) rtnValue = ((buf.st_mode & _S_IFREG) != 0);
#else
  struct stat 	buf;
  i = stat(path, &buf);
  if (i == 0) rtnValue = (S_ISREG(buf.st_mode));
#endif
