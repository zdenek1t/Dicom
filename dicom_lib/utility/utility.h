...
** Last Update:		$Author: smm $, $Date: 2002/02/26 21:13:44 $
...
void
UTL_GetDicomDate(char *date);
void
UTL_GetDicomTime(char *time);
...
CTNBOOLEAN
UTL_IsDirectory(const char* path);
CTNBOOLEAN
UTL_IsFile(const char* path);
CONDITION
UTL_DeleteFile(const char* path);
CONDITION
UTL_ScanDirectory(const char* path, LST_HEAD** lst);

CONDITION
UTL_SetConfigFile(const char* configFile);
CONDITION
UTL_TestConfigFile(const char* configFile);
char*
