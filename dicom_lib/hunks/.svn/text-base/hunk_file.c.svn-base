...
HF_Create(char *fname, int num_hunks, int hunk_record_length, int num_recs_per_hunk)
{
    ...
    int      	  wb,			/* the  number of bytes written */
    ...
    if ((GS_hunkfd = open(fname, O_RDWR | O_CREAT, 0777)) < 0) return (HF_CREATERROR);
...
HF_Open(char *fname)
{
    int        addsize, hunksize, hs, seek_pos, rb;

    if (Opened == 1) return (HF_OPENERROR);
    if ((GS_hunkfd = open(fname, O_RDWR)) < 0) return (HF_OPENERROR);
